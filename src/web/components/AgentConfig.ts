...
import { setBtnBusy, LIVE_ONLY_OPS, LIVE_ONLY_RUNTIMES, RUNTIMES, runtimeLabel } from "./Settings";
...
// 런타임 교체(swap-runtime) 대상 화이트리스트 — activation.ts RUNTIMES(정본, 서버가 최종 검증)와 동일 4종.
// b3os_native는 스왑 대상 아님(서버가 invalid_runtime 400 반환) — select에서부터 제외.
...
export function renderAgentConfig(root: HTMLElement): void {
  ...
  const load = async (agentId: string) => {
    ...
    try {
      ...
      renderInto(root, data, () => load(agentId)); // reload 콜백: 재적용/재시작 후 config 뷰 in-place 재렌더(첫화면 튕김 방지)
    } catch (e) {
...
function renderInto(root: HTMLElement, data: ConfigResponse, reload: () => void) {
  const { agent, persona } = data;
  const customPersona = (data.custom_persona ?? "").trim(); // pre-fill = SOUL.md 정본(서버가 SOUL 에서 읽어 내려줌). agents.json fallback 제거 — purpose 필드 자체가 없어졌다(OWNER 2026-07-17)
  ...
  // 레지스트리 표에 자연스럽게 이어지는 icon 행 — 아이콘 클릭 시 피커 펼쳐 아이콘+색 변경(색은 펼침 메뉴 안에 녹임, OWNER 2026-06-27).
  const iconRow = `
      <tr class="border-b border-surface-3/50">
  ...
  const swapTargetOptions = RUNTIMES
    .filter((r) => SWAP_TARGETS.includes(r.value) && r.value !== agent.runtime && (LIVE_ONLY_OPS || !LIVE_ONLY_RUNTIMES.has(r.value)))
    .map((r) => `<option value="${escape(r.value)}">${escape(r.label)}</option>`)
    .join("");
  root.innerHTML = `
  ...
  // 멘션명(별칭) — 별도 버튼 없이 아래 '저장' 버튼이 role·persona 와 함께 저장(OWNER 2026-07-19). PATCH /members/:id {nicknames}.
  ...
  tokenBtn?.addEventListener("click", async () => {
    ...
    // 엉뚱한 팀원에게 적용 방지 — 팀원명 크게 노출한 확인창(OWNER 2026-07-01).
  ...
  swapBtn?.addEventListener("click", async () => {
    ...
    // 파괴적 작업(런타임 정지→재기동) — 팀원명 크게 노출한 확인창(offboard/토큰변경과 동일 패턴, OWNER 2026-07-01 관례).
    if (!(await confirmModal(pick(
  ...
  offBtn?.addEventListener("click", async () => {
    // 이름-타이핑 가드에 더해 명시적 확인창 — 엉뚱한 팀원 퇴사 방지, 팀원명 크게(OWNER 2026-07-01).
    if (!(await confirmModal(pick(
    ...
    if (offMsg) {
      // openclaw는 게이트웨이 정리(bootout+프로필 제거)로 퇴사가 더 오래 걸림 → 눈에 띄게 안내(OWNER 2026-07-02).
