...
function renderInto(root: HTMLElement, data: ConfigResponse, reload: () => void, runtimeOptions: RuntimeOption[] = FALLBACK_RUNTIME_OPTIONS) {
  ...
  // 공개빌드에선 아래 '런타임 교체' 섹션 전체를 렌더하지 않는다(LIVE_ONLY_OPS=false) — 공개판은 UI에서만 swap 숨김(OWNER 0721). 서버 엔드포인트는 유지되며 codex target·미준비는 publicRuntimeGate/runtime_not_ready 가 막는다.
