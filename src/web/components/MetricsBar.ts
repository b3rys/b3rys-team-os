...
export function renderMetricsBar(root: HTMLElement): void {
  const update = () => {
    ...
    // 리셋된다(OWNER 2026-06-24 '메뉴 클릭하면 좌로 튀네' — 오른쪽으로 스크롤해 누르면 맨 왼쪽으로 점프).
    ...
    // Claude Max 사용통계(호출수·토큰) 표시는 노이즈라 제거(OWNER 2026-07-01). maxGauge 자리표시자만 유지(항상 빈값).
    const maxGauge = "";
    ...
    const macAppDownload = isMacClient()
      ? `<a id="b3os-app-download"
            href="${apiBase()}/B3rys-unsigned-test.zip"
            download
    ...
    // Mobile: minimal (OWNER + agent count + CPU + Ollama + alerts). Desktop: + RAM + Load.
    root.innerHTML = `
      <div class="relative">
        <!-- ★바에 overflow-hidden 을 걸면 안 된다★ — Tasks/Inbox/Doc 드롭다운이 absolute 로 바 아래에
             펼쳐지므로 통째로 잘린다 (OWNER 2026-07-14 실사용 발견). 넘침 클립은 ★우측 지표 클러스터에서만★ 한다. -->
    ...
    // → 시스템 브라우저(Safari)로 넘겨 받게 한다(OWNER 2026-07-02, reports 다운로드와 동일 패턴).
    ...
    // 언어 깃발 토글(우상단 구석) — ko↔en 전환 + 즉시 반영. persist 후 reload로 전 컴포넌트 재렌더(OWNER 2026-07-01).
    ...
    // 네비 드롭다운 hover-open (OWNER R6) — 마우스 오버 시 펼침, 벗어나면 닫힘. 상태가 실제로 바뀔 때만
    ...
    // 팀이름(브랜드) 클릭 = 홈 = Tasks 칸반 기본 표시 (OWNER 2026-06-21 R4).
