...
interface Member { id: string; display_name: string; role: string; runtime: string; avatar_emoji?: string; icon?: string | null; icon_color?: string | null }
...
// state: 'blocked' = 회복 가능한 대기(예: preflight 미로그인 — OWNER 터미널 로그인 후 자동 통과). 'failed'와 달리 OT 전체를 실패로 보지 않는다.
...
// export — AgentConfig.ts(런타임 교체 UI)가 select 옵션+라벨을 그대로 재사용(단일 소스, 값 중복 방지).
export const RUNTIMES: { value: string; label: string }[] = [
  { value: "claude_channel", label: "Claude Channel (tmux)" },
  { value: "openclaw", label: "OpenClaw (gateway)" },
  { value: "hermes_agent", label: "Hermes" },
  { value: "codex", label: "Codex CLI (live-only)" },
  { value: "b3os_native", label: "b3os Native (API direct)" },
];
...
// 첫 세팅 락(OWNER): 팀명+팀장ID 통과해야 팀원 영입. GET /settings가 setup_complete/lead_actor_id 반환.
...
// 라이브 전용 운영 버튼(전체 핵심룰 재적용 + 롤백) 노출 여부. 공개 빌드에선 이 기능을 빼고 라이브에서만 동작(OWNER 2026-06-30).
// make-public-release.sh 가 아래 true→false 로 뒤집어 공개판 Settings 에선 버튼이 렌더되지 않는다(백엔드 엔드포인트는 PUBLIC_BUILD 가드).
export const LIVE_ONLY_OPS = false; // PUBLIC-BUILD-FLIP:LIVE_ONLY_OPS (public build: live-only ops hidden)
// codex·b3os_native = 라이브 검증 후 공개(OWNER 2026-07-05). 공개빌드(LIVE_ONLY_OPS=false)에선 영입·스왑 런타임 선택지에서 숨김.
// 공개 런타임 = claude_channel·openclaw·hermes_agent 만.
export const LIVE_ONLY_RUNTIMES = new Set(["b3os_native", "codex"]);
...
export function runtimeLabel(rt: string): string {
  return RUNTIMES.find((r) => r.value === rt)?.label ?? rt;
}
...
// 설정 뷰 재진입 시 Slack 상태/헬스 재조회 — 토큰 갱신·재연동 후에도 '재설치 필요' 배지가 stale로 남던 것 방지(OWNER).
...
function recruitFormHtml(): string {
  return `
    <div class="rounded-lg border border-accent-green/35 bg-surface-2/60 p-3.5 mt-2">
      <div class="text-[13px] font-semibold text-slate-100 mb-3">${pick("신규 영입 — 새 팀원 OT", "Onboard — new member OT")}</div>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div><label class="${labelCls}">${pick("런타임", "Runtime")}</label><select id="rec-runtime" class="${inputCls} dash-select">${RUNTIMES.filter((r) => LIVE_ONLY_OPS || !LIVE_ONLY_RUNTIMES.has(r.value)).map((r) => `<option value="${r.value}">${escape(r.label)}</option>`).join("")}</select></div>
        <div class="md:col-span-2"><label class="${labelCls}">${pick("멘션 별칭", "Mention aliases")} <span class="text-slate-600">${pick("(선택 · 쉼표 구분 · @멘션 라우팅용. 비우면 id+이름 자동)", "(optional · comma-separated · for @mention routing. Empty = auto from id+name)")}</span></label><input id="rec-nicknames" class="${inputCls}" placeholder="${pick("예: member, forin, member", "e.g. member, forin, member")}" autocomplete="off" /></div>
      ...
      <div class="flex items-center gap-3 mt-3">
        <button id="rec-submit" class="${btnPrimary}">${pick("영입 시작", "Start onboarding")}</button>
...
function otStepperHtml(): string {
  ...
  const joinedGreeting = joined ? `
       <div class="rounded-lg border border-accent-green/30 bg-accent-green/10 p-3 mt-3 text-[12px] text-accent-greenSoft leading-relaxed">
         ...
         ${m.runtime === "claude_channel" ? `<div class="mt-1 text-[11px] text-slate-400">${pick("첫 claude 팀원은 봇 DM의 6자리 페어링 코드를 사람이 승인하는 게 정상입니다(2번째 팀원부터는 자동 승계). 봇에 '안녕'을 보내 6자리 코드가 오면 아래에서 승인하세요.", "For the first claude teammate, manually approving the 6-digit pairing code from the bot DM is the normal path (later teammates inherit automatically). Say 'hi' to the bot, then approve below when the 6-digit code arrives.")}</div>` : ""}
       </div>` : "";
  const footer = joined
    ? `${joinedGreeting}
...
function otZoneHtml(): string {
  ...
  return `<button id="rec-open" class="mt-2 w-full ${btnGhost} py-2.5 border-dashed">+ ${pick("영입", "Onboard")}</button>`;
}
...
function wireSystemOp(): void {
  ...
  // (접근제어/PIN은 System OP에서 제거 — OWNER 2026-06-28. 라우터/저장/봇확인은 바로 동작.)
  ...
  const identity = card(pick("팀 정체성", "Team identity"), "team identity", `
    <div class="space-y-4">
      ...
      <div>
        ...
        <input id="set-lead-id" data-req="1" class="${inputCls}" maxlength="40" value="${escape(_settings.lead_id)}" placeholder="${pick("예: owner", "e.g. owner")}" />
...
function wire(): void {
  ...
  // 언어 토글은 우상단 헤더 깃발(MetricsBar #locale-flag)로 이전 — 여기선 제거(OWNER 2026-07-01).
  // 저장 + 필수 필드 검증(팀 이름·팀장 ID·팀장 이름) — OWNER 2026-07-10
  ...
  // 🔄 전체 재시작 — member·정지팀원 제외 전원 재시작(member 맨 마지막).
  ...
  // 🔴 전체 정지 (비상) — member 제외 전원 정지. 더블컨펌(강한 경고).
...
function wireOtZone(): void {
  ...
  // id 검증 — blur 시 형식(백엔드 ID_RE 미러)·중복 위반이면 빨간 테두리 + 영입 버튼 비활성(자동변환 안 함, OWNER 2026-07-01).
  {
    ...
    const validateRecId = (showRed: boolean) => {
      ...
      recId.style.borderColor = bad && showRed ? "rgb(var(--status-blocked))" : ""; // 빨간 테두리는 blur 시(OWNER)
  ...
  // 인증 다시 확인(preflight recheck) — OWNER가 터미널에서 로그인한 뒤 즉시 재점검. 서버가 checkRuntimeAuth 재실행 후 OT 갱신.
  // (자동 폴링도 같은 일을 하지만, OWNER가 방금 로그인했을 때 1.5s 안 기다리고 바로 확인하는 단축 버튼.)
  ...
  _root.querySelector<HTMLButtonElement>("#ot-pair-approve")?.addEventListener("click", async () => {
    ...
    try {
      ...
      if (j.ok) {
        ...
        refreshOtZone();
        // openclaw는 pairing으로 join(폴링 종료 경로 밖) → 검수 테스트(acceptance-check) 트리거 + 로스터 재조회를 여기서 직접 호출. codex/claude/hermes는 폴링 종료 시 아래 pollOt가 refreshMembers()를 하지만 openclaw는 pair-approve가 폴링을 멈춰서 둘 다 누락됐음(로스터 stale → F5 필요). OWNER 2026-07-01.
...
// OWNER가 터미널 로그인해도 '다시 확인'을 수동으로 눌러야만 활성화 버튼이 뜬다. throttle(AUTO_RECHECK_MS)로
