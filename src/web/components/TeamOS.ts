...
function scheduledRow(s: TeamOsScheduled): string {
  ...
  const srcTag = s.source === "openclaw_cron" ? "openclaw" : "launchd";
...
function renderInto(root: HTMLElement, snap: TeamOsSnapshot | null): void {
  ...
  // TEAM OP = the dedicated Telegram capture bot, NOT a team member. Its liveness ≈ team-collab
  // running (the capture worker is embedded in that server).
  // team-collab 서비스 식별: launchd prefix(com.<user>)가 머신마다 다르므로 하드코딩 대신
  // 접미사(.team-collab) + service kind 로 매칭한다(포터빌리티 — 웹 번들엔 server의 teamosLaunchdPrefix 를
  // import 못 함: process.env.USER 미존재 + server-only deps). 접미사는 teamosProbe 가 `${prefix}.team-collab` 로 생성.
  const teamCollab = snap.scheduled.find((s) => s.kind === "service" && s.label.endsWith(".team-collab"));
  const opActive = teamCollab?.running === true;
  const infraBlock = `
    <div class="rounded-md border border-amber-500/30 bg-amber-500/5 p-3">
      <div class="flex items-center gap-2 mb-1">
        <span class="text-base">🛰️</span>
        <span class="text-sm font-semibold text-slate-100">TEAM OP — @example_team_op_bot</span>
        ...
        ${opActive
          ...
          : `<span class="inline-flex items-center gap-1 text-[11px] text-slate-300"><span class="w-2 h-2 rounded-full bg-slate-500"></span>${pick("상태 미상", "Status unknown")}</span>`}
      </div>
