...
type JobSource = "launchd" | "openclaw_cron" | "scheduled_job";
...
function sourceLabel(source: JobSource): string {
  if (source === "scheduled_job") return "DB scheduled_job";
...
export function renderJobsView(root: HTMLElement): void {
  ...
  function bodyHtml(): string {
    ...
    if (jobs.length === 0) {
      return `<div class="flex-1 flex items-center justify-center text-slate-500 text-sm">${pick("등록된 launchd/DB scheduled_job/openclaw cron job이 없습니다.", "No launchd/DB scheduled_job/openclaw cron jobs registered.")}</div>`;
    }
      ...
      <div class="flex-1 overflow-y-auto p-4">
        <div class="mb-4 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p class="mt-1 text-sm text-slate-500">${pick("/api/teamos scheduled snapshot을 종류별로 묶어 보여줍니다. DB scheduled_job의 next/last는 KST로 표시합니다. 실행·중지·등록·삭제는 없습니다.", "Groups the /api/teamos scheduled snapshot by kind. DB scheduled_job next/last times are shown in KST. No start, stop, register, or delete.")}</p>
          </div>
