    ...
    root.innerHTML = threads
      .map((t: Thread) => {
        ...
        const partsIcons = t.participants
          .map((p) => {
            const iconName = knownAgentIds.has(p) ? agentIconName(p) : "user";
            return `<span class="text-accent-greenSoft">${renderIcon(iconName, { size: 12 })}</span>`;
          })
          .join("");
        return `
          <button data-thread-id="${t.id}"
            ...
            <div class="flex items-center justify-between gap-2 mb-1">
              <div class="flex items-center gap-1.5 min-w-0">
                ...
                <span class="flex items-center gap-0.5">${partsIcons}</span>
              </div>
