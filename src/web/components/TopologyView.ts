...
//                          "미완(오래됨)" sub-badge so it never paints the node red. (v1.1 OWNER fix)
...
// Node PRIMARY color = the team member's reachability, per OWNER's v1.1 feedback
...
export function renderTopology(root: HTMLElement): void {
  ...
  // 3-layer vertical layout (v1.2, OWNER): ① interface (top) → ② bus (middle) → ③ members (bottom).
