    ...
    routing: `
      ...
      ${section(pick("통신 원리", "Communication principles"), "canonical viewer", `
        ...
        <div class="mt-3 grid grid-cols-1 gap-3 lg:grid-cols-4">
          ...
          ${policyCard(pick("취합과 개별보고", "Collection vs individual"), pick("취합은 한 명이 모아 한 번만 보고합니다. 개별 보고 지시는 각자가 GD에게 직접 보고합니다.", "Collection reports once through the collector. Individual-report requests are reported directly to GD by each member."), true)}
