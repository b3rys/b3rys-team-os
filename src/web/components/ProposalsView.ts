...
function hoursSince(s: string): number {
  ...
  // (없으면 new Date가 로컬 타임존(KST)으로 해석 → 정체 경과가 9시간 부풀려짐, GD 발견 2026-07-04.)
    ...
    <div class="mb-4 rounded-md border border-status-idle/40 bg-status-idle/10 px-4 py-3">
      <div class="mb-2 flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <div class="mt-0.5 text-[11px] text-slate-500">${pick("최종 승인/반려는 actor=gd로만 기록됩니다.", "Final approve/reject is recorded only as actor=gd.")}</div>
        </div>
        <div class="flex flex-wrap gap-2">
          <button data-gd-decision="accepted" class="rounded-md border border-accent-green/40 bg-accent-green/15 px-3 py-1.5 text-xs font-semibold text-txt-green hover:bg-accent-green/25">${pick("승인", "Approve")}</button>
          <button data-gd-decision="rejected" class="rounded-md border border-status-blocked/40 bg-status-blocked/14 px-3 py-1.5 text-xs font-semibold text-txt-red hover:bg-status-blocked/24">${pick("반려", "Reject")}</button>
        </div>
      </div>
...
export function renderProposalsView(root: HTMLElement): void {
  ...
  let detailScroll = 0; // 상세 패널 스크롤 보존(폴링 full-render 시 위로 튐 방지, GD 2026-06-25)
  ...
  function render() {
    ...
    root.querySelectorAll<HTMLButtonElement>("[data-gd-decision]").forEach((b) =>
      b.addEventListener("click", () => {
        if (!detail) return;
