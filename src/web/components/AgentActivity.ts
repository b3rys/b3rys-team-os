...
import { apiBase, sendMessage } from "./../ws";
...
function renderInto(root: HTMLElement, agent: Agent, msgs: Message[], myId: string) {
  ...
  root.innerHTML = `
    <div class="flex-1 flex flex-col min-h-0">
      ...
      <form class="border-t border-surface-3 p-3 flex gap-2" id="agent-activity-form">
        <input type="text" id="agent-activity-input"
          ...
          placeholder="${pick(`${escape(agent.display_name)} 에게 메시지 (Enter 전송)`, `Message ${escape(agent.display_name)} (Enter to send)`)}" autocomplete="off" />
        <button type="submit"
          class="px-3 py-1.5 rounded-md bg-accent-btn text-accent-on text-sm font-semibold hover:bg-accent-btnHover">${pick("전송", "Send")}</button>
      </form>
    </div>`;
  const body = root.querySelector<HTMLElement>("#agent-activity-body");
  if (body) body.innerHTML = items;
  const form = root.querySelector<HTMLFormElement>("#agent-activity-form");
  const input = root.querySelector<HTMLInputElement>("#agent-activity-input");
