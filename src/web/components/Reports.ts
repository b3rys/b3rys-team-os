...
//   parseSqliteDate 가 Z 를 붙여 UTC 로 고정한다 — OWNER 가 2026-07-04 에 만든 단일 출처인데 ★여기만 안 썼다.★
...
function renderList(): void {
  ...
  _root.innerHTML = `
    <div data-reports-list-scroll class="h-full overflow-y-auto">
      <div class="max-w-3xl mx-auto px-4 md:px-6 py-5 pb-20">
        ...
        ${items.length ? `<div class="flex flex-col gap-2.5">${cards}</div>${loadMore}` : empty}
      </div>
    </div>`;
  const scroller = _root.querySelector<HTMLElement>("[data-reports-list-scroll]");
  ...
  _root.querySelectorAll<HTMLElement>(".reports-card").forEach((el) => {
    el.addEventListener("click", () => { rememberListScroll(); _curId = el.dataset.id || null; _curType = null; _view = "detail"; renderDetail(); });
    el.addEventListener("keydown", (e) => {
      if (isInteractiveTarget(e.target)) return;
      if (e.key === "Enter" || e.key === " ") { e.preventDefault(); rememberListScroll(); _curId = el.dataset.id || null; _curType = null; _view = "detail"; void renderDetail(); }
    });
  });
...
async function renderDetail(): Promise<void> {
  ...
  const showForm = async (type: string): Promise<void> => {
    ...
    // 새창보기 — 현재 form 타입을 새 탭에서 전체화면으로(좁은 대시보드 뷰 대신, OWNER 2026-06-22).
    ...
    if (dl) {
      ...
      // 써서 형식이름으로 떨어지던 버그 fix (OWNER R5 4737). 한글 보존(\p{L}).
      ...
      // 복귀 불가(OWNER 2026-07-02). 새창보기와 동일하게 시스템 브라우저로 넘겨 다운로드(대시보드 화면 유지).
