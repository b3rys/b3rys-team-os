...
/**
 * send.sh argv 조립(순수함수 — 테스트 대상). ★direct_to_gd는 명시 true일 때만 --direct-to-owner 붙인다★:
 * 본문 기반 자동승격 영구 금지(OWNER 2026-07-09 — 위임 본문 오발화로 direct_to_gd 폭주했던 회귀 방지).
 */
export function buildSendArgs(input: SendMessageInput, actor: string): string[] {
  ...
  if (input.direct_to_gd === true) a.push("--direct-to-owner");
  return a;
}
...
export function buildMcpServer(db: Database): McpServer {
  ...
  // recall_dms — 특정 멤버의 OWNER 1:1 최근 DM (recallDmMessages 재사용, 멤버별 격리).
  reg(
    "b3os_recall_dms",
    {
      title: "b3os DM recall",
      description: "연결 신원 본인의 OWNER 1:1 최근 DM(최신순, 기본 10건·limit로 조정)을 반환한다. ★멤버별 격리★ — 본인만, 타 멤버 지정은 거부(lead 예외).",
      inputSchema: {
        agent_id: z.string().min(1).optional().describe("조회 대상(생략=연결 신원 본인). 본인만 허용(lead 예외 별도)"),
