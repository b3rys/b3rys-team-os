 ...
 *
 * ★요구사항★: 대화에는 승인창이 뜨지 않는다(0회이지 1회가 아니다). 셸을 안 타면 이 경로를 안 지난다.
 *
 * ★그 뒤 실행 모드가 "never" 로 바뀌어 승인 팝업 자체가 안 생긴다★(appServerRunner.ts).
 *   그래도 이 방식을 유지하는 이유는 승인이 아니라 ★인용★ 이다 — 답이 명령 문자열에 안 들어가므로
 *   따옴표·백틱·heredoc 종료자가 해석될 자리가 없다. 그 결함군은 승인과 무관하게 남아 있었다.
 *
