 ...
 *  - read-only 샌member스: 모든 쓰기/실행이 escalation → 승인요청 → 게이트가 봄(F1으로 non-Tier-D도 ask=거절). 실효 O.
 *  - ★workspace-write 샌member스: writable_roots 안 파괴(rm·overwrite·git reset)는 codex가 승인요청 없이 자체 실행 →
 ...
 *  - ask(위험하지만 정당) → needsApproval=true → 상위(M5)가 OWNER 텔레그램 팝업.
 */
...
export interface ApprovalJudgment {
  ...
  needsApproval: boolean; // true면 OWNER 팝업 필요(M5)
...
export function judgeApproval(
  ...
  // ask → OWNER 팝업 필요. 팝업 전엔 확정 안 함(상위가 처리).
...
function evaluate(agent: PermissionAgent, req: ApprovalRequest, ctx: PermissionContext): PermissionCheck {
  ...
  if (req.method === "execCommandApproval" || req.method === "item/commandExecution/requestApproval") {
    ...
    // → resolveWithoutPopup=fail-closed denied(팝업 전) / M5 팝업(OWNER 승인). 정책 'ask=거절'이 셸을 실제로 덮는다.★
    if (c.tier === "allow") {
...
/**
 * ★OWNER UX(2026-07-05): 하드 차단은 막다른 "차단됨"이 아니라 사용자에게 '어떻게 하면 되는지' 안내한다.★
 * ★OWNER 후속(msg838): 터미널 명령뿐 아니라 파일변경·외부전송 등 작업 종류별로 안내가 달라야 한다.★
...
export function actionGuidance(req: ApprovalRequest): string | null {
  ...
  return [
    "이 작업은 안전상 제가 직접 할 수 없어요(외부 전송·삭제·권한 변경 등은 OWNER 확인이 필요).",
    `요청 내용: ${what}`,
    "필요하시면 OWNER가 직접 하시거나, 안전한 대안으로 다시 지시해 주세요.",
    caution,
  ].join("\n");
}
