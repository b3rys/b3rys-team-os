...
// 첫 접촉(영입 후 첫 인사) 판정을 ★영속★ 마커로 한다(OWNER 2026-07-10 버그픽스).
...
function scheduleToolPrompt(input: { text: string; agentId: string; teamBaseUrl: string; repoRoot: string }): string {
  return [
    ...
    "Command contract:",
    `bun run ${input.repoRoot}/scripts/schedule-reminder.ts --base-url ${input.teamBaseUrl} --agent ${input.agentId} --created-by ${input.agentId} --body "<reminder body>" (--delay-seconds <seconds> | --run-at <iso>) --direct-to-owner`,
...
export async function runScheduleReminderCli(
  ...
  const args = [
    ...
    ...(req.direct_to_gd !== false ? ["--direct-to-owner"] : []),
  ];
...
// ★team-comm owner-gate (OWNER 2026-07-09, 설계 §3a): 그룹서 owner 아닌데 native로 답하는 것 방지.
...
export async function handleMessage(
  ...
  // ★team-comm group native deny (OWNER 2026-07-09, 설계 §3a, Codex F1): 그룹(chatId<0) native 처리를 막는다.
  ...
  // ★예약 등록은 LLM 판단으로만★ (OWNER 2026-07-05): 이전엔 여기서 키워드(isOneShotScheduleRequest) 매치만으로
  // buildDirectScheduleRequest → 즉시 등록하고 턴을 종료했다. 그 결과 "3분뒤 메시지가 안왔네" 같은 ★불평·질문★도
  // 시간패턴+행동패턴만 있으면 자동 예약돼버림(LLM이 의도를 판단하지 못함). OWNER 지적대로 이건 파싱이지 판단이 아니다.
  ...
  const toolAwareText = scheduleRequest
    ...
    : text;
  // 첫 접촉(여태 한 번도 인사 안 한 신입) = 영입 후 첫 응답 → 인사 + OT 받은 것 언급하며 시작(OWNER 2026-07-01).
  //   판정은 ★영속 마커★(재시작에도 남음) — 인메모리 prior(세션 resume용)와 분리해, 이미 합류한 팀원이
  //   재시작 후 재소개하지 않게 한다(OWNER 2026-07-10 버그픽스). prior는 아래 resumeSessionId 로만 쓴다.
...
export async function runBridge(deps: BridgeDeps = {}): Promise<void> {
  ...
  const live: BridgeDeps = {
    ...
    // (2026-07-05 OWNER 테스트에서 "덱스 있어?"조차 dead-end로 발견). Tier-D(danger-full-access)는 이 grant로도
  ...
  //   건강한 브리지를 '미기동'으로 오판했다(BUG5, OWNER 맥북테스트 2026-07-03). getMe는 즉시 반환+토큰/도달성
