 ...
 *
 * OWNER 방침: exec 폴백 없음 → app-server 예외를 여기서 정면 처리(에러=ok:false로 실패통지 경로 태움).
...
/**
 * ★M5.3: db 있으면 팝업(ask→OWNER 텔레그램 승인), 없으면 fail-closed denied.★
...
async function runViaAppServer(opts: CodexTurnOptions, db?: Database): Promise<CodexTurnResult> {
  ...
  try {
    ...
    await client.startThread({
      ...
      // ★쓰기 지원은 M5 팝업(OWNER 승인) 배선 후 별도 설계. 그 전엔 안전 우선으로 read-only 고정.★
      sandbox: "read-only",
    ...
    // 최종 답변에 붙인다. 막다른 "차단됨" 대신 OWNER가 직접 실행할 수 있게(우리 규칙 Tier-D=OWNER 터미널만 정합).★
    const guidances: string[] = [];
    const r = await client.runTurn(opts.prompt, {
      onApproval: async (req) => {
        // ★승인요청 → permissionGate 판정. Tier-D=denied 확정. ask면: db 있으면 OWNER 팝업(M5.3), 없으면 fail-closed denied.★
        const j = judgeApproval(permAgent, req, permCtx);
        let decision: ReviewDecision;
        if (!j.needsApproval) {
          decision = j.decision ?? "denied"; // Tier-D deny 확정
        } else if (db) {
          decision = await requestApprovalPopup(db, req, permAgent.id, opts.cwd); // ★ask→OWNER 텔레그램 팝업★
        } else {
