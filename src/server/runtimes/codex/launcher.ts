...
export function renderMinimalCodexConfig(_p: Pick<CodexBridgePaths, "workdir">): string {
  // ★주석에도 정책 키 이름을 쓰지 않는다.★ 시험이 "이 파일에 정책이 없다" 를 문자열로 재는데,
  //   주석에 키 이름이 들어 있으면 그 시험이 주석 때문에 실패한다(2026-08-14 실제로 걸렸다).
  return [
    "# Generated by b3rys team-collab. Do not copy host ~/.codex/config.toml here.",
    "# 실행 모드는 이 파일이 아니라 appServerRunner 가 codex 프로토콜로 명시한다(openclaw 와 같은 방식).",
    "",
  ].join("\n");
}
