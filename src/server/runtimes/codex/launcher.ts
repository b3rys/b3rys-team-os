 ...
 *
 * ★`approval_policy = "on-request"` — 묻는 게 기본이다.★ (팀 리드 2026-08-12)
 * 터미널/앱에서 codex 를 쓰면 경계 밖은 ★물어보고 사람이 누른다.★ 그게 원래 모습이고,
 * 우리가 하는 일은 그 물음을 ★텔레그램 그 팀원 방으로 옮기는 것★ 이다(appServerPopup).
 * 한때 `"never"` 로 막아둔 적이 있는데, 그건 옮겨온 게 아니라 ★기능을 뺀 것★ 이었다.
 *
 * TOML 주의: `default_permissions` 는 ★첫 섹션 헤더보다 위★ 여야 한다. 아래로 내려가면
 * `[sandbox_workspace_write].default_permissions` 가 되어 ★조용히 무시된다.★
 */
export function renderLockedDownCodexConfig(p: Pick<CodexBridgePaths, "workdir">): string {
  return [
    ...
    `# workspace root: ${p.workdir}`,
    'approval_policy = "on-request"',
    `default_permissions = ${tomlString(MEMBER_PERMISSION_PROFILE)}`,
    "",
    `[permissions.${MEMBER_PERMISSION_PROFILE}]`,
    'extends = ":workspace"',
    "",
    // 팀 버스(send.sh)가 curl 로 오간다 — 막으면 팀원이 답을 못 보낸다(실측: curl exit 7).
    `[permissions.${MEMBER_PERMISSION_PROFILE}.network]`,
    "enabled = true",
    "",
    `[permissions.${MEMBER_PERMISSION_PROFILE}.filesystem]`,
    ...HOME_DENY_PATHS.map((d) => `${tomlString(d)} = "deny"`),
    "",
    // 작업 트리 안은 쓰기. 단 시크릿류는 ★읽기도★ 막는다(claude settings.json deny 목록과 동일 대상).
    `[permissions.${MEMBER_PERMISSION_PROFILE}.filesystem.":workspace_roots"]`,
    '"." = "write"',
    ...SECRET_DENY_GLOBS.map((g) => `${tomlString(g)} = "deny"`),
    "",
  ].join("\n");
}

/** 프로파일 이름은 ★멤버 공통★ 이다 — dex 전용이 아니다(다른 팀원도 이 런타임으로 영입 가능). */
export const MEMBER_PERMISSION_PROFILE = "b3os-member";

/** 작업 트리 밖에서 통째로 막는 홈 디렉토리(자격증명 저장소). */
export const HOME_DENY_PATHS = ["~/.ssh", "~/.aws"] as const;

/** 작업 트리 ★안에서도★ 막는 시크릿 파일. claude settings.json 의 deny 목록과 같은 대상. */
export const SECRET_DENY_GLOBS = [
  "**/.env",
  "**/.env.*",
  "**/*credential*",
  "**/*secret*",
