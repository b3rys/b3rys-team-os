 ...
 *
 * ★codex 설정을 그대로 따른다 — 우리가 덧대지 않는다.★ (팀 리드 2026-08-12)
 *
 * 한때 `[permissions.*]` 프로파일로 경로 deny 목록을 얹었는데, ★실측 결과 그 프로파일이 승인 자체를
 * 무력화했다★ — 프로파일이 있으면 사람이 '허용' 을 눌러도 codex 가 열어주지 않는다(accept 도,
 * acceptWithExecpolicyAmendment 도 안 통함). 프로파일을 빼니 승인이 바로 통했다.
 * 즉 이 버전에서는 ★경로 deny 목록과 승인 escalation 이 양립하지 않는다.★
 *
