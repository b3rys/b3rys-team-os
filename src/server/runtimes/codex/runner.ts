 ...
 *
 * 출처: codex-channel-poc(2026-06-27 OWNER 검증)에서 이식. 검증된 gotcha 반영:
...
// 아키텍처/OS 무관(하네스, OWNER 2026-07-02): env 미설정 시 bare 이름 → PATH lookup(OPENCLAW_BIN 패턴과 동일). Apple-Silicon 고정경로 하드코딩은 Intel/Linux서 spawn 실패.
...
export function buildCodexArgs(opts: CodexTurnOptions, lastMsgFile: string, sandbox = opts.sandbox ?? "read-only"): string[] {
  ...
  // codex의 flag로 해석돼 read-only 샌member스 탈출=호스트 RCE. "--" 뒤는 무조건 PROMPT.
...
export async function runCodexTurn(opts: CodexTurnOptions): Promise<CodexTurnResult> {
  ...
  if (opts.agentId) env.OWNER_AGENT_ID = opts.agentId;
  if (!env.TEAM_DB_PATH) {
