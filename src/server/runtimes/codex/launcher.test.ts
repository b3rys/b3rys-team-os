...
describe("codex launcher (M4) — 순수 렌더러", () => {
  ...
  test("★codex 본래 모델로 간다★ — 묻고, 누르면 실제로 열린다", () => {
    const cfg = renderLockedDownCodexConfig({ workdir: "/tmp/cody workspace" });
    // 실측: [permissions.*] 프로파일이 있으면 사람이 허용을 눌러도 codex 가 열어주지 않는다.
    // 경로 deny 목록과 승인 escalation 은 이 버전에서 양립하지 않는다 → codex 설정을 그대로 따른다.
    expect(cfg).toContain('sandbox_mode = "workspace-write"');
    expect(cfg).toContain('approval_policy = "on-request"');
    expect(cfg).not.toContain("[permissions."); // 프로파일을 다시 넣으면 승인이 죽는다
  });

  test("작업 폴더는 쓰기 · 팀 버스(curl)는 열어둔다", () => {
    const cfg = renderLockedDownCodexConfig({ workdir: "/tmp/cody workspace" });
    expect(cfg).toContain('writable_roots = ["/tmp/cody workspace"]');
    expect(cfg).toContain("network_access = true"); // 막으면 send.sh 로 답을 못 보낸다(실측 curl exit 7)
  });
