...
describe("codex launcher (M4) — 순수 렌더러", () => {
  ...
  test("최소 config: host trust/MCP/plugins 를 상속하지 않는다", () => {
  ...
  test("★우리 실행 정책을 config 에 쓰지 않는다★ — dex 만 잠겨 있던 원인(2026-08-14)", () => {
    ...
    expect(cfg).not.toContain("writable_roots");
    expect(cfg).not.toContain("network_access");
    expect(cfg).not.toContain("[permissions."); // 프로파일을 다시 넣으면 승인이 죽는다(2026-08-12 실측)
  });

  test("ensureCodexHome: host config.toml 을 복사하지 않고 최소 config 를 생성", () => {
    const tmp = join(tmpdir(), `codex-home-test-${process.pid}-${Math.random().toString(36).slice(2)}`);
    ...
    expect(cfg).not.toContain("sandbox_mode"); // ★정책은 프로토콜이 정한다★
    expect(cfg).not.toContain("mcp_servers");
    expect(cfg).not.toContain("plugins");
    rmSync(tmp, { recursive: true, force: true });
  });
