...
import { codexBridgeLaunchdLabel, codexBridgePaths, renderLaunchWrapper, renderBridgePlist, ensureCodexHome, renderLockedDownCodexConfig, persistOwnerChatIdIfEmpty, MEMBER_PERMISSION_PROFILE, HOME_DENY_PATHS, SECRET_DENY_GLOBS } from "./launcher";
...
describe("codex launcher (M4) — 순수 렌더러", () => {
  ...
  test("locked-down config: host trust/MCP/plugins 를 상속하지 않는다", () => {
  ...
  test("★default_permissions 는 첫 섹션 헤더보다 위여야 한다★ — 아래면 TOML 이 남의 섹션 키로 먹는다", () => {
    // 실제로 당한 자리다: `[sandbox_workspace_write]` 뒤에 붙였더니
    // `sandbox_workspace_write.default_permissions` 가 되어 ★에러 없이 무시★ 됐다.
    const lines = renderLockedDownCodexConfig({ workdir: "/tmp/cody" }).split("\n");
    const keyAt = lines.findIndex((l) => l.startsWith("default_permissions"));
    const firstSectionAt = lines.findIndex((l) => l.trimStart().startsWith("["));
    expect(keyAt).toBeGreaterThanOrEqual(0);
    expect(firstSectionAt).toBeGreaterThanOrEqual(0);
    expect(keyAt).toBeLessThan(firstSectionAt);
  });

  test("시크릿은 작업 트리 안에서도 막는다 + 팀 버스(curl)는 열어둔다", () => {
    const cfg = renderLockedDownCodexConfig({ workdir: "/tmp/cody" });
    // 작업 트리 안이라고 .env 까지 열어주면 deny 목록의 의미가 없다.
    for (const g of SECRET_DENY_GLOBS) expect(cfg).toContain(`"${g}" = "deny"`);
    for (const d of HOME_DENY_PATHS) expect(cfg).toContain(`"${d}" = "deny"`);
    // 네트워크를 막으면 팀원이 send.sh 로 답을 못 보낸다(실측: curl exit 7).
    expect(cfg).toContain("enabled = true");
    expect(cfg).not.toContain("network_access = false");
  });
  ...
  test("ensureCodexHome: host config.toml을 복사하지 않고 locked-down config를 생성", () => {
    ...
    expect(cfg).toContain("# workspace root: /tmp/cody");
    expect(cfg).not.toContain("mcp_servers");
    expect(cfg).not.toContain("plugins");
    rmSync(tmp, { recursive: true, force: true });
  });
