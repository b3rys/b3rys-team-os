/**
 * M5 — codex app-server 승인요청 → OWNER 텔레그램 팝업(기존 승인 인프라 재사용).
 *
 * ★재빌드 아님:★ permissionGate.requestPermission(팝업 생성)+getPermissionRequest(상태) + telegramCapture(3버튼 렌더)
 * 를 재사용. onApproval이 ask면 여기서 팝업 띄우고 OWNER 결정을 폴링해 ReviewDecision으로 매핑한다.
 *
...
const POPUP_TTL_MS = Number(process.env.B3OS_CODEX_APPSERVER_POPUP_TTL_MS ?? 60 * 60 * 1000); // 1h (OWNER: 무응답→hold)
...
/** M5.2 — permission_request 상태를 폴링해 OWNER 결정을 ReviewDecision으로. 무응답 TTL→denied(hold). */
