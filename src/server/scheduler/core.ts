...
export const WEEKLY_SHARED_CURATION_BODY = [
  ...
  "★완료 후 OWNER에게 직접 정리 보고: send.sh --direct-to-owner. 내용 = SHARED.md 에 올린 항목 건수·주요 내용. 없으면 '이번 주 없음' 한 줄.",
].join("\n");
export const WEEKLY_SELF_LEARNING_BODY = [
  ...
  "등록 방법: POST /api/proposals — 각 proposal 에 근거(왜)+예상효과 필수. 등록하면 OWNER 리뷰 게이트로 올라갑니다(자동 적용 아님).",
  "억지로 만들지 마세요 — 진짜 팀에 필요한 것만. 없으면 등록하지 말고 '이번 주 없음'. 정책·보안·라우팅·외부전송 규칙 변경도 proposal 로만(자동 적용 금지).",
  "★완료 후 OWNER에게 직접 정리 보고: send.sh --direct-to-owner. 내용 = 등록한 proposal 목록(각 제목·유형[룰/과제/이슈]·근거 한 줄·proposal ID). 없으면 '이번 주 없음' 한 줄.",
].join("\n");
