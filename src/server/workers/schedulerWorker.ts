...
export function startSchedulerWorker(db: Database): () => void {
  const enabled = process.env.B3OS_SCHEDULER_ENABLED !== "false";
  const dryRun = process.env.B3OS_SCHEDULER_DRY_RUN === "1";
  const intervalMs = Math.max(5_000, Number(process.env.B3OS_SCHEDULER_INTERVAL_MS ?? DEFAULT_INTERVAL_MS));
  if (!enabled) {
    console.log("[scheduler_worker] disabled (B3OS_SCHEDULER_ENABLED=false)");
    return () => undefined;
  }
