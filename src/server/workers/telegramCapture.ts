...
export function startTelegramCapture(deps: CaptureDeps): () => void {
  ...
  async function registerCommands(): Promise<void> {
    const locale = getLocale(deps.db);
    // ★공개판 자동완성 게이트: 내부 명령(approve·digest·deploy)은 publicBuild 에서 자동완성 목록에도 안 뜨게.
    //   핸들러·op메뉴는 이미 slashCommands(PUBLIC_BUILD) 로 게이트됨 — setMyCommands 목록도 같은 소스로 일치시킨다.
    const allowed = new Set(slashCommands().map((c) => c.slice(1)));
    const commands = [
    ...
    ].filter((c) => allowed.has(c.command));
