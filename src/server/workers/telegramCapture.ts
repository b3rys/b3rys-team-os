...
export function shouldUseNativeCodexOpenclawPath(
  ...
  // 경계 lookahead 유지(조사 비포함): "@코덱스가 ..." 는 native 가 못 잡으므로 false(기존 동작 보존).
...
// ── 슬래시 명령(팀op 운영성 응답, GD 2888) formatter — 순수 DB read ───────────
...
// /menu — GD 승인 대기 목록 + 액션 카탈로그. (2026-06-10) Stage 1: 표시만, 승인은 대시보드(인증)에서.
export function fmtMenu(db: Database, opts: { publicBuild?: boolean } = {}): string {
  const locale = getLocale(db);
  const owner = pick(locale, "팀장", "the team lead");
  const pending = listApprovals(db, "pending");
  const publicBuild = opts.publicBuild ?? PUBLIC_BUILD;
  const out = [publicBuild ? pick(locale, "🛰️ 운영 메뉴", "🛰️ Operations menu") : pick(locale, `🔐 ${owner} 승인 메뉴`, `🔐 ${owner} approval menu`)];
  if (publicBuild) {
    out.push(pick(locale, "\n공개판에서는 안전한 읽기·제어 명령만 표시합니다.", "\nThe public build only shows safe read/control commands."));
  ...
  out.push(pick(locale, `\n명령: ${slashCommands(publicBuild).join(" · ")}`, `\nCommands: ${slashCommands(publicBuild).join(" · ")}`));
  return out.join("\n");
}
...
export function startTelegramCapture(deps: CaptureDeps): () => void {
  ...
  const SLASH_CMDS = slashCommands();
  ...
  function approvalAllowedIds(): string[] {
    ...
    // ★폴백(Bill 하네스검증 #8): claude-only 팀은 detect-lead-id 를 돌리기 전까지 lead_telegram_id 가 비어
    //   approvalAllowedIds()=[] → 모든 @all·승인 콜백이 fail-closed 로 "권한 없음" 막힘. owner_chat_id(=팀장
    //   텔레그램 user id, claude 페어링/설정에서 이미 채워짐)를 인가 폴백으로 쓴다. 빈 값이면 아무것도 안 더해
    //   fail-closed 불변식(empty=deny-all)은 유지. 음수 그룹 chat_id 는 콜백 from.id(양수 user id)와 일치할 수
    //   없으니 오인가 방지로 숫자 user id(^\d+$)만 받는다.
  ...
  // /approve — 승인 대기 항목을 [✅승인][❌거절] 인라인 버튼으로. PIN 없이 GD 탭=인증(2026-06-10 GD).
  ...
  async function handleCallback(cb: NonNullable<TgUpdate["callback_query"]>): Promise<void> {
    ...
    if (oo) {
      ...
      const target = deps.agents().find((a) => a.id === agentId);
      if (want && isAgentOff(agentId) && isTeamOfficialMember(target) && activeOfficialMemberCount(deps.agents(), isAgentOff) >= MAX_OFFICIAL_TEAM_MEMBERS) {
        const text = pick(locale, `공식 팀원 ${MAX_OFFICIAL_TEAM_MEMBERS}명 상한입니다. 기존 팀원을 정지하거나 퇴사 처리해 주세요.`, `The ${MAX_OFFICIAL_TEAM_MEMBERS}-member limit is reached. Disable or offboard an existing member first.`);
        await tg("answerCallbackQuery", { callback_query_id: cb.id, text, show_alert: true });
        appendAuditFile("capture", "member_limit_blocked", agentId, { limit: MAX_OFFICIAL_TEAM_MEMBERS });
        return;
      }
  ...
  async function handleSlashCommand(text: string, msgId?: string, targetChat: string = GROUP_ID): Promise<boolean> {
    ...
    const body =
      cmd === "/menu" ? fmtMenu(deps.db) : // ★/menu 배선 — 승인대기+액션카탈로그+실행상태(하네스/Steve: fmtMenu가 데드코드였음). GD 2026-07-02
  ...
  async function handle(
    ...
    // 가시성 Stage A: GD 그룹 메시지를 공유 버스 스레드에 적재 → 모든 에이전트가 같은 맥락을 읽음.
    ...
    appendAuditFile("capture", "route_decision", text.slice(0, 200), {
      ...
      needsGdConfirm: decision.needsGdConfirm,
    ...
    //  - outcome=route 만 (closure/ask_gd 는 깨우지 않음)
    //  - bill 도 주입 대상(2026-06-02 requireMention=true 전환 — specialist 와 동일하게 owned 만 받음)
    //  - codex도 @코덱스 같은 alias 호출은 gateway session injection (실제 @bot_username 은 plugin 처리라 skip)
  ...
  async function runInjection(
    ...
    text: string,
      ...
      for (const id of decision.targetAgentIds) {
        ...
        // 이전엔 @코덱스/Codex-답장이면 OpenClaw native 가 처리하라고 capture 주입을 skip 했는데,
        // native 경로엔 owner-gate 가 없어 reply_to_bot implicit 활성화로 @빌 owner 메시지에도 codex 가 반응했다.
        // → B: codex OpenClaw groupPolicy 'open'→'disabled'(native 그룹 처리 off) 와 한 쌍으로,
        //   이제 codex 의 모든 owner 케이스(@코덱스/Codex-답장/sticky)를 capture 가 라우터 owner 판정 후 bridge 로만 주입한다.
        //   owner 가 codex 가 아니면(예: @빌) targetAgentIds 에 codex 없음 → 미주입(=침묵).
  ...
  async function loop(): Promise<void> {
    ...
    while (!stopped) {
      try {
        ...
        // 새 승인/권한 요청을 GD op DM에 즉시 push(매 폴 사이클). 독립 try/catch로 폴러 격리(하네스 Q6: push 경로 DB 오류가 메인 폴 사이클 못 흔들게).
