// GD 1:1 DM sync 워커 — 30초 주기로 각 런타임 저장소의 GD 1:1을 dm_message로 옮긴다(준실시간).
...
const DEFAULT_INTERVAL_MS = 10_000; // GD 2026-07-09: 싱크 주기 10초. env B3OS_DM_SYNC_INTERVAL_MS로 조정(최소 5초).

// 런타임별 GD 1:1 저장소 경로 해석. 멤버 id 와 런타임 내부 profile/agent id 가 다를 수 있으므로
    ...
    if (!warnedNoOwner) {
      ...
      console.warn(
        "[dm_sync] 팀장 chat_id 를 찾지 못해 1:1 DM 캡처를 건너뜁니다 " +
          "(설정 owner_chat_id · claude 페어링 · GD_CHAT_ID 전부 없음). 대시보드 설정에서 채우세요.",
      );
    }
