// approvals — OWNER 승인 큐 API. 권한 액션을 PIN 승인으로 처리(터미널 0). (2026-06-10 OWNER)
...
// 승인 v2(OWNER 2026-07-08): 에이전트(승인자 풀)가 merge 승인/거절 시 신청자에게 버스 통지(읽기전용).
...
export function createApprovalsApp(deps: ApprovalsDeps): Hono {
  ...
  // 승인 v2(OWNER 2026-07-08): 에이전트(승인자 풀)가 merge_to_main 을 승인/거절하는 경로(OWNER 텔레그램 탭과 별개).
  //   보안: canApproveTier(자기승인 금지·풀 멤버십). merge_to_main 전용 — 다른 액션은 OWNER 탭 유지(일반 승인시스템 불변).
  //   승인 성공 시 executeApproval(OWNER 탭과 동일 실행경로 — merge-to-main.sh nonce 흐름 재사용) + 신청자 버스 통지.
  app.post("/approvals/:id/agent-approve", async (c) => {
    ...
    if (row.action_key !== "merge_to_main") return c.json({ ok: false, error: "이 경로는 merge_to_main 전용(다른 승인은 OWNER 탭)" }, 400);
    ...
    // ★Devon 리뷰 #1: params.tier 를 실제로 읽는다. core 머지는 풀이 승인 불가(OWNER --lead 직접머지 경로). tier 고정 금지.
    const tier = params.tier === "core" ? "core" : "normal";
    if (tier === "core") return c.json({ ok: false, error: "core tier 머지는 풀 승인 불가 — OWNER 직접머지(--lead) 경로" }, 403);
  ...
  app.post("/approvals/:id/agent-reject", async (c) => {
    ...
    if (tier === "core") return c.json({ ok: false, error: "core tier 머지는 풀 처리 불가 — OWNER 경로" }, 403);
    const auth = canApproveTier({ tier, approver, author, isLead: false, normalApprovers: getNormalApprovers(db) });
