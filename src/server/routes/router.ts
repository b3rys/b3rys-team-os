 ...
 * 캡처 브리지(텔레그램 그룹→여기)와 injection 레이어는 GD 승인 후 단계 연결.
 */
export function createRouterRoutes(deps: RouterRouteDeps): Hono {
  ...
  r.post("/route", async (c) => {
    ...
    // 결정 audit — self-learning 감사추적 (나중에 GD 교정과 함께 튜닝 소스).
    ...
    // (ON 단계: outcome=route 면 targets 를 tmuxInject/openclawBridge 로 깨움 — GD 입회 후.)
    return c.json({
      ...decision,
      confidentOwner,
      suppress,
