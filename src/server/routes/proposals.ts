...
// (2026-07-01 GD: 공개 팀에서도 유령배정 없이 동작하게 심플화. 하드코딩 codex/bill 제거.)
  ...
  if (status === "gd_report") {
    ...
    return {
      ...
      description:
        ...
        `주의: accepted/rejected 최종 전이는 actor=gd만 가능하다.\n` +
...
async function notifyGdReportReached(db: Database, proposalId: string, _agents: AgentRecord[]): Promise<void> {
  ...
  if (isTestProposalFixture(p)) {
    auditGdReportNotice(db, "gd_report_notice_skipped_test", proposalId, {
      ...
      title: p.title,
      source: p.source,
      reason: "test proposal fixture must not notify GD",
    });
    return;
  }
...
export function createProposalRoutes(deps: ProposalRouteDeps): Hono {
  ...
  r.delete("/proposals/:id", async (c) => {
    ...
    const current = getProposal(deps.db, proposalId) as { proposal?: { proposer_agent?: string } } | null;
    if (!current) return c.json({ error: "unknown_proposal" }, 404);
    if (actor !== leadActorId() && actor !== current.proposal?.proposer_agent) {
      return c.json({ error: "proposal archive는 proposer_agent 또는 팀장만 가능" }, 403);
    }
  ...
  r.post("/proposals", async (c) => {
    ...
    const claimedProposer = String(body.proposer_agent ?? "").trim();
    if (auth.actor.actor !== leadActorId() && claimedProposer !== auth.actor.actor) {
      return c.json({ error: "proposer_agent must match authenticated actor" }, 403);
    }
