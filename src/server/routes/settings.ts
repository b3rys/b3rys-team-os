...
// 공개빌드에선 런타임 교체(swap-runtime)를 ★웹 UI에서만★ 숨긴다(AgentConfig LIVE_ONLY_OPS 게이트) — 서버 엔드포인트는 유지(OWNER 2026-07-21, "UI만 숨겨").
...
export function createSettingsApp(deps: SettingsDeps): Hono {
  ...
  app.post("/members/:id/swap-runtime", async (c) => {
    // 공개빌드에선 런타임 교체를 웹 UI에서만 숨긴다(OWNER 0721) — 서버 엔드포인트는 유지. codex target·미준비는 아래 publicRuntimeGate 가 막는다.
    const id = c.req.param("id");
  ...
  // 실제 설치/복사는 안 함(OWNER 2026-06-30 옵션 A) — 스킬은 repo-local skills 공유 경로라 런타임 무관하게 그대로 실행.
