...
describe("★독촉 본문 — 개별보고면 무시하라고 알림이 직접 말한다★", () => {
  ...
  it("★진짜 수집(전원 답함, 종합 안 나감)에는 탈출구를 안 붙인다★ — 거기 붙이면 진짜 수집이 무시된다", () => {
    ...
    // ★탈출구는 붙이되 ★조건부★ 여야 한다★
    //   원래 이 자리는 "탈출구를 아예 안 붙인다" 였다. 이유는 무조건적 탈출구
    //   ("종합할 게 없으니 무시하세요")가 진짜 수집까지 무시하게 만들기 때문이다 — 그 우려는 여전히 옳다.
    //   그런데 반대편 피해가 실측됐다: 이미 보고한 사람이 이 문구를 보고 ★한 번 더 보냈다★
    //   (steve, 보고 83초 뒤 독촉 수신). 서버는 팀장 1:1 DM 을 못 보므로 ★보고 여부를 모른다.★
    ...
    // ★무조건 탈출구는 여전히 금지★ — 개별보고 분기의 그 문구가 여기 새면 원래 우려가 되살아난다.
