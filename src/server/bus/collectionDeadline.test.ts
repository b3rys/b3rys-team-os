 ...
 *
 * ★팀원 스스로는 못 한다★: hermes·openclaw·codex 는 턴이 끝나면 ★프로세스가 죽는다★ —
 ...
 *   → ★질문은 답보다 먼저 나간다★ 가 유일하게 믿을 수 있는 신호다.
 */
...
/**
 * ★"다 왔는데 종합이 안 나갔다" — 전원 응답 뒤에도 종합이 발송되지 않는다.★ (2026-07-14 라이브 실측)
 *
 * ═══ codex 가 35분째 멈췄다 ═══
...
describe("★다 왔는데 종합이 안 나갔다 — 그것도 막힌 것이다★", () => {
  ...
  it("★개별보고 — 일부만 direct_to_gd 로 답했고 나머지는 아직이어도 안 깨운다★ (① narrowing 완성, 일부 응답 오탐)", () => {
    // 라이브 오탐: 개별보고에서 한 기여자는 direct_to_gd 로 답했는데 다른 기여자가 늦자 → 미응답 독촉이 나갔다.
...
/**
 * ★윈도 경계 오탐 — 이미 끝난 수집에 [마감] 이 갔다.★ (2026-07-13 라이브 실측)
 *
 * ═══ 실측 ═══
 ...
 *
 * ★수집자는 룰을 지켰다. 서버가 훼손된 시야로 미응답이라고 판정했다.★
...
describe("★윈도 경계 — 훼손된 시야로 멀쩡한 팀원을 고발하지 않는다★", () => {
  ...
  it("★윈도 경계 케이스 재현: 위임이 윈도 밖으로 밀려나도 ack 을 질문으로 세지 않는다★", () => {
 ...
 *
 * 2026-07-13 사고: 마감 체커가 한 팀원을 ★47번★ 재촉했다.
 ...
 *   (2026-07-17 라이브: 개별보고 위임 뒤 독촉이 나갔고 무시됐다. 무시가 맞지만, 팀원이 룰을 뒤져
...
describe("★--individual 플래그 — 뿌리는 순간 개별보고를 안다★", () => {
   ...
   *   오남용의 결말은 ★맨 위 실측과 동일한 무한대기★ 다 — 답 하나가 안 와서 수집이 정지하고 조용히 죽는다.
...
describe("★독촉 본문 — 개별보고면 무시하라고 알림이 직접 말한다★", () => {
   ...
   *  sweep 의 catch 가 그걸 삼켜 ★woke=0 이 조용히 나온다★.
  ...
  it("★진짜 수집(전원 답함, 종합 안 나감)에는 탈출구를 안 붙인다★ — 거기 붙이면 진짜 수집이 무시된다", () => {
    ...
    // ★탈출구는 붙이되 ★조건부★ 여야 한다★
    //   ★무조건적 탈출구는 진짜 수집까지 무시하게 만든다★
    //   ("종합할 게 없으니 무시하세요"). 그 위험은 여전히 있다.
    //   그런데 반대편 피해가 실측됐다: 이미 보고한 사람이 이 문구를 보고 ★한 번 더 보냈다★
    //   (보고 83초 뒤 독촉 수신). 서버는 팀장 1:1 DM 을 못 보므로 ★보고 여부를 모른다.★
    //   해법은 탈출구를 빼는 것도, 무조건 붙이는 것도 아니라 ★"이미 했다면" 이라는 조건을 다는 것★ 이다.
    //   아직 안 한 사람에게는 해당되지 않으므로 진짜 수집은 그대로 재촉된다.
    expect(body).toContain("아직 종합 보고를 안 하셨으면 지금 보내세요");   // 기본 동작은 무조건
    expect(body).toContain("이미");                                        // 탈출구에 조건이 붙는다
    expect(body).toContain("이 알림은 무시하세요");
    // ★순서 고정★ — 기본 동작이 먼저, 탈출구는 뒤. 되돌리면 애매한 팀원이 무조건 무시할 확률이 오른다.
    expect(body!.indexOf("지금 보내세요")).toBeLessThan(body!.indexOf("이 알림은 무시하세요"));
    // ★무조건 탈출구는 여전히 금지★ — 개별보고 분기의 그 문구가 여기 새면 진짜 수집을 무시하게 만드는 위험이 되살아난다.
    expect(body).not.toContain("종합할 게 없으니");
  });
...
/**
 * ★팀장 1:1 DM(dm_message)은 보고 근거로 쓰지 않는다★
 *
 * 이 테스트들은 ★그 결정을 고정한다★ — 누가 근거로 넣으면 여기서 잡힌다.
...
// ═══ ★알림 첫 줄이 "어느 수집인지" 를 말한다★ ═══
//   thread_id 는 원래도 실려 나갔지만 `tg--1003947108339` 처럼 ★사람이 못 읽는다.★
//   실측: 한 팀원이 하루에 이 알림을 4번 받았고, 매번 어느 건인지 되짚어야 했다.
describe("★알림 식별 — 제목 + msg id★", () => {
  ...
  it("★제목은 아무것도 벗기지 않는다★ — 벗기려다 대괄호 짝이 깨졌다", () => {
...
// ★식별이 ack 을 집으면 안 된다★ (2026-08-04 · P1)
...
it("★44자 경계에 이모지가 걸려도 반쪽 문자가 안 남는다★", () => {
  ...
  //   정상 이모지도 서로게이트 ★쌍★ 이라 /[\uD800-\uDFFF]/ 에 걸린다(★그래서 이 정규식만으로는 못 가른다★).
