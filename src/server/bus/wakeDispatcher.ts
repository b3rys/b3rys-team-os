 ...
 * - BUS_DISPATCH_ENABLED default ON (OWNER 2026-07-19); set =false → shadow mode: logs decisions, sends no wakes.
...
// direct_to_gd DM 릴레이용 — OWNER 1:1 DM chat_id(setting owner_chat_id). 미설정이면 undefined(릴레이 불가).
...
// ★기본 ON★ (OWNER 2026-07-19): 협업/위임은 dispatcher 가 팀원을 깨워야 성립한다. 명시적으로 "false" 일 때만 shadow.
...
//                      → expired, OWNER report never marked delivered (2026-06-29 diagnosis). A timeout
...
// 동적 보강 allowlist 파일(2026-06-10 OWNER): 영입 시 재시작 없이 에이전트를 wake 대상에 추가하기 위함.
...
//   운영 allowlist(lui/devon/…)에 밀려 allowlist_not_enabled 되던 격리 갭(Codex 진단, 테스트 격리 게이트). OWNER 2026-07-01.
  ...
  return {
    async wake(targetAgentId, row, teamContext): Promise<WakeResult> {
      ...
      try {
        ...
        // OWNER 1:1 DM 에 자기 reply 도구로 직접 보고하도록 directReport 를 넘긴다.
 ...
 * OWNER 에게 직접 visible 응답하라"고 표시한 directed 메시지인지 판별. 핵심은 자연어 추측이 아니라 라우팅 계약:
 * 발신자(LLM)가 meta.reply_mode="direct_to_gd" 를 붙인다. 타겟은 ★OWNER 1:1 DM(owner_chat_id)★ — 팀방 유무 무관.
 * 수신자는 본문 해석 없이 이 플래그만 보고 OWNER DM 에 직접 응답한다. (source_thread_id 는 무시 — DM 이 기본.)
 * tg- thread 로 이미 온 메시지엔 적용 안 함(이미 텔레그램 경로). 반환: {groupId=owner DM chat_id, threadId} or null.
 */
// ★2026-07-08 OWNER: direct_to_gd 기본 타겟 = OWNER 1:1 DM(owner_chat_id). 그룹 아님.★
//   이유: 팀방 없는 퍼블릭 사용자도 릴레이 가능해야 하고, OWNER-facing 보고를 그룹에 노출하면 footgun.
//   반환 groupId 필드 = 이제 "OWNER DM chat_id" (봇이 자기 토큰으로 그 DM에 post). source_thread_id 는 무시(호환).
 ...
 *   → 팀장이 단톡방에서 시킨 일을 member이 member에게 재위임하면, member는 tg- 를 물려받아
...
export function resolveDirectToGd(row: PendingDispatchRow, ownerChatId?: string): { groupId: string; threadId: string } | null {
  ...
  //   DM 까지 보내면 ★중복 보고★ 다. (OWNER 2026-07-14 확인: "둘 다 상관없어" → 중복 없는 쪽을 택함.
...
function openclawDeliveryRecorder(
  ...
  return (info: { ok: boolean; text: string; groupId: string; deliveryMessageId?: string | number | null }) => {
    recordReportDelivery(db, {
      ...
      recipient: directToGd ? "owner" : info.groupId,
      threadId,
      refId,
      body: info.text,
      ok: info.ok,
...
function makeOpenclawAdapter(db: Database, agents: () => AgentRecord[]): WakeAdapter {
  return {
    async wake(targetAgentId, row, teamContext): Promise<WakeResult> {
      ...
      // openclaw_agent_id(게이트웨이 프로필명)는 실제 레지스트리 레코드에서 해석한다 — codex="owner",
      // devon="devon" 등 agents.json 에 이미 박혀 있음(이전 하드코딩 targetAgentId==="codex"?"owner" 대체).
      ...
      const agent: AgentRecord = {
        ...
        // hasCapability(agent,"native_routing") 로 codex 의 공유 env 파일(owner 프로필 토큰) fallback 을 켜기
        // 때문 — 이게 빠지면 codex 봇 토큰을 못 찾아 postTelegramAsOpenclaw 가 {ok:false}(agent_reply_post_failed)
        // 로 실패해 direct_to_gd 보고가 OWNER 그룹에 안 올라간다(2026-06-24 continuation-driver 라이브 진단).
      ...
      if (resolveThreadKind(row.thread_id) !== "telegram_group" || isTeammateDirected(row)) {
        // case 6 (direct_to_gd): Bill 이 위임하며 "OWNER 그룹에 직접 응답" 플래그를 단 directed 메시지면,
        // 버스 ack 대신 OWNER 1:1 DM 에 visible reply 로 올린다. (injectOpenclawTelegramTurn 가 답을 DM 에 전송)
        const directToGd = resolveDirectToGd(row, ownerDmChatId(db));
        if (directToGd) {
          try {
            ...
            const wakePromise: Promise<boolean | typeof timeoutSentinel> = injectOpenclawTelegramTurn({
              ...
              hopCount: row.hop_count,
              directReport: true,
              kind: "direct_to_gd", // ★봉투 kind★ — 이 경로는 OWNER 직보(hermes hReplyRoute 의 direct_to_gd 와 동일)
            });
            ...
            if (result) {
              ...
              // miss the delivered report → re-wake → the agent re-reports = DUPLICATE OWNER DM report. A DB audit
        ...
        // 버스로 ack/응답한다. (기존 inbox-only 는 directed handoff 가 openclaw 에이전트한테 안 닿는 버그였음 — OWNER 2489)
  ...
  return {
    async wake(targetAgentId, row, teamContext): Promise<WakeResult> {
      ...
      try {
        ...
        //   말하려면 팀원이 ★직접 보낸다★ — send.sh --to <상대> / --to broadcast(방) / --direct-to-owner.
        //   ★"보낸 것만 말한 것이다."★  → 침묵에 토큰이 필요없고, 수신자를 서버가 추측하지 않는다.
        //   (claude 가 원래 이렇게 돌고 있었고, 그래서 이 병이 없었다.)
        // ★답이 어디로 가야 하는지는 ★여기가 안다★. 주입문이 추측하게 두지 않는다.★ (OWNER 2026-07-14)
        ...
        await runHermesTeamTurn({
          ...
          // ★상한은 한 곳(hermesBridge)에만 산다★ (OWNER 2026-07-15). 호출부마다 넘기면 또 슬랙처럼 빠뜨린다.
        });
...
/**
 * ★네가 이미 무엇을 보냈는지 ★보이게★ 만든다.★ (OWNER 2026-07-13: "룰과 세션데이터로 하자")
 *
 * ═══ 무엇이 잘못됐었나 (실측) ═══
 ...
 *   ★유사도 같은 걸로 서버가 막지 않는다★ (OWNER). ★팀원이 볼 수 있으면 팀원이 판단한다.★
 */
...
const CTX_HOURS = Number(process.env.CTX_HOURS ?? 24);   // ★24시간. 넘으면 문맥 없음★ (OWNER 결정)
const CTX_MSG_CHARS = Number(process.env.CTX_MSG_CHARS ?? 800);
const CTX_TOTAL_CHARS = Number(process.env.CTX_TOTAL_CHARS ?? 8000);
// ★단톡방(그룹 스레드) 전용 상한★ (OWNER 2026-07-16): 그룹방은 스레드 하나에 전 과제가 섞여, 12건·24h 를
...
// ★참고용 주입은 '자기것만' · 5건 (그룹·버스 통일, 분기 없음)★ (OWNER 2026-07-16 "전부 5개로 해. 분기 타지 말고").
...
export function buildTeamContext(db: Database, threadId: string, agentId?: string): string {
  try {
    ...
    if (agentId) {
      // ★자기것 + 나에게 온 것만 · 5건 (그룹·버스 통일, 분기 없음)★ (OWNER 2026-07-16 "전부 5개, 분기 타지 말고").
      //   from=나(내 팬아웃·발언) OR to=나(기여자 답·OWNER 지시)만. 남의 딴-대화(예: codex→demis 리뷰 팬아웃)를 제거.
    ...
    // ★잘림이 진짜 답을 잘랐다★ (OWNER 질문: "메시지가 크면?"). 실측: 최근 121건 중 4건이 200자 초과인데
    ...
    for (let i = recent.length - 1; i >= 0; i--) {   // 최신부터 담고, 예산 다 쓰면 옛 것을 버린다
      ...
      // ★언제 일인지 안 알려주고 있었다.★ (OWNER 2026-07-13: "오래된걸 주면 안좋은거 아냐?")
    ...
    const summary =
      ...
      // ★24시간 넘은 이력은 안 붙인다 — 대신 ★꺼내 보는 법★ 을 한 줄로 알려준다 (OWNER 지시).★
...
function buildDispatchPlan(
  ...
  // ack-only reply wake-gate (team-comm 왕복 축소, OWNER 2026-07-09):
  ...
  // ack-loop guard (team-comm ②, OWNER 2026-07-09; 하네스 적대검증 반영 2026-07-10):
  ...
  //   막음 → 반드시 '최근 시간창(WINDOW_MIN)' bound. (2)source=agent 만(user/system=OWNER 지시 절대 안 막음).
  //   (3)broadcast 제외(@all wake-all 보장 침해 방지). shadow-first(로그만) → OWNER 재검토+재검증 후에만 enforce.
  ...
  if (
    ...
    row.source === "agent" &&                 // user/system(OWNER 지시)은 절대 억제 안 함
  ...
  // OWNER-report reminder (prompt-injection, OWNER 2026-07-11): while this collector has an active team-lead
  ...
  // 2026-05-27 (OWNER): agents NOT in the allowlist are expired (dropped), not requeued.
  ...
  // @all-gating (OWNER 2026-05-27): a broadcast message wakes every recipient ONLY when its body
  ...
  // → 기여자 둘이 각각 답해도 hermes 는 ★자기를 깨운 한 건만★ 보였다 → "member 1건뿐" → ★종합 불가.★
  ...
  // ★그룹방도 푼다.★ (OWNER 2026-07-13: "그룹방도 풀면 안돼?")
  //   ═══ 게이트를 걸 이유가 없었다 ═══
...
//   3경로 모두 동일 terminal expire로 닫아 중복 가시응답 방지(OWNER 2026-06-03 dup root cause). preservesInbox.
...
function recordDispatchOutcome(
  ...
  if (outcome.exception !== undefined) {
    ...
    // Generic retry here re-wakes + re-posts → duplicate visible replies (OWNER 2026-06-03 dup root cause —
  ...
  if (lastError === UNKNOWN_SIDE_EFFECT_DETAIL) {
    // 2026-05-27 (OWNER): "애매하면 만료" — partial inject is ambiguous (may have partially
...
export function startWakeDispatcher(deps: WakeDispatcherDeps): () => void {
  ...
  // Startup cleanup: expire stale user-message pending recipients (2026-05-30, OWNER).
  ...
  // Startup cleanup: expire stale 'wake_dispatched' zombies (2026-05-31, OWNER).
  ...
  async function tick(): Promise<void> {
    try {
      ...
      const now = Date.now();
      // ★마감 독촉(collectionDeadline) — 개별보고 제외로 좁혀 되살림★ (OWNER 2026-07-15)
      ...
      //   구별 = 기여자가 collector 에게 direct_to_gd 없이 답했나(수집) vs OWNER 께 direct_to_gd(개별).
      ...
      // Team-Collect close tick (OWNER 2026-07-11, feature-flag OFF → no-op unless enabled): close any due
