...
describe("3c wake 경계 — comm 매트릭스", () => {
  ...
  // 그래서 이 테스트은 "팀원이 @all 로 전원을 깨운다" 를 지키고 있었다 — 룰에 근거가 없는 동작이다.
  ...
  // ★통일이 동작으로 고정되는 지점 — 이 테스트이 없으면 인라인 정규식을 되넣어도 아무것도 안 깨진다.★
  ...
  // 이 테스트이 없어서 한 번 헤맸다. 축에 테스트이 없으니 'system 깨움이 죽었다' 를
