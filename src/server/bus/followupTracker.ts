 ...
 * existing owner concept: `meta.reply_mode === "direct_to_gd"` (the `--direct-to-owner` flag), which is
 ...
 *   1. The requester flags the send with `--direct-to-owner --expect-report-by <duration>` → the
...
/** The existing "destined for the team lead" signal (the `--direct-to-owner` flag / owner routing). */
...
/**
 * ★자가등록 (OWNER 2026-07-18 응답가드)★ — 팀원이 "작업이 길어져 팀장 보고를 잊으면 안 된다" 싶을 때 스스로 건다.
 ...
 * · 게이트 = 등록자 runtime 이 one-shot(턴기반) — OWNER: "턴기반 팀원만". 탈락은 ★명시적 reason 으로 거절★한다 (침묵 금지).
 ...
 */
export const SELF_FOLLOWUP_DEFAULT_DURATION = "10m"; // OWNER: 10분 = 팀 작업 기준시간
...
function hasSubstantiveReport(db: Database, row: PendingFollowupRow): boolean {
  ...
  // re-wakes → the one-shot agent re-reports = DUPLICATE OWNER DM report (harness B, HIGH: fires on ~every success).
...
export function checkPendingFollowups(db: Database, opts: CheckPendingFollowupsOpts = {}): CheckPendingFollowupsResult {
  ...
  for (const row of due) {
    ...
    const body =
      `[follow-up] 팀장님께 보고하기로 한 게 아직 안 왔음 — 지금 팀장님께 보고해줘 (--direct-to-owner).` +
  ...
  // GC pass (2026-07-10, OWNER): the report can land AFTER the single re-wake fired — clean those rows
