/**
 * ★마감 — 답이 안 오면 그때 상황으로 보고하게 깨운다.★ (OWNER 2026-07-13)
 *
 * ═══ 왜 필요한가 (팀장 라이브 테스트) ═══
 ...
 * 그런데 ★다시 깨워주는 게 없으니 거기서 멈췄다.★ 팀장은 "member 응답 대기중" 만 본다.
 *
 * ═══ OWNER 원칙 ═══
 *   "5분/10분 정도 되면 그냥 ★그때 상황으로 응답★ 을 주면 될텐데 심플하게..
 *    문제는 ★그 다음 wake 되면 그거에 맞게 다시 응답★ 주면 되고. 오픈클로·member도 같은 원칙으로."
 *
...
export function findStalledCollections(db: Database, agents: AgentRecord[]): StalledCollection[] {
  ...
  for (const C of collectors) {
    ...
    for (const { thread_id } of threads) {
      ...
      // C 에게 들어온 메시지 (답·지시)
      // ★direct_to_gd 답은 제외한다★ (2026-07-15, OWNER) — 기여자가 `--to <collector>` 로 보냈어도
      //   reply_mode=direct_to_gd 면 그건 ★개별보고(OWNER 행)★ 지 collector 를 위한 수집 기여가 아니다.
      //   (2026-07-15 라이브: 서귀포 개별보고에서 hermes·codex 가 `--to steve --direct-to-owner` 로 보내자
      ...
      // 'owner' 리터럴에 대하여:
      //   아래 `IN ('user','broadcast','owner')` 의 'owner' 는 옛 owner agent-id 표기의 잔재다. 리포트는
      //   'user'(owner DM)·'broadcast'(방) 로 가고 이 둘이 실제 케이스를 다 커버하므로 'owner' 는
      //   매칭되지 않는 무해한 leftover다. 게다가 바로 아래 `NOT IN (targets)` 절이 '기여자가 아닌
      //   어디로든' 을 이미 잡으므로 'owner' 없이도 동작은 동일하다(하위호환을 위해 남겨 둠).
          ...
          `SELECT COUNT(*) AS n FROM message
            WHERE thread_id = ? AND from_agent_id = ? AND created_at > ?
              AND (to_agent_id IS NULL
                   OR to_agent_id IN ('user','broadcast','owner')
      ...
      for (const t of targets) {
        ...
        const toRequester = requester
          ...
          : false;
        // ★direct_to_gd 로 답한 것도 '답함' 이다 — ★개별보고(OWNER 행)★.★ (2026-07-15, OWNER)
        //   기여자가 `--to <collector> --direct-to-owner` 로 보내면 목적지는 OWNER 다(inbound 에선 이미 제외됨).
      ...
      // ★개별보고면 전원/일부 무관하게 통째 스킵★ (2026-07-15 ①, OWNER) — 기여자가 direct_to_gd 로 OWNER께 답하기
      ...
      // ★--individual = 확정이 아니라 '사실로 반박 가능한 힌트' 다.★ (dbak 리뷰 2026-07-17, OWNER 지시로 도입)
      //
      //   위임자가 `send.sh --individual` 로 뿌렸다 = "각자 OWNER께 직접 보고해라". 종합할 사람이 없으니 안 깨운다.
    ...
    try {
      ...
      const body = c.missing.length === 0
        ...
        : `[마감] ${COLLECTION_DEADLINE_MIN}분이 지났는데 아직 보고가 없습니다. ` +
          ...
          `(각자 OWNER께 직접 보고하도록 시킨 건이면 종합할 게 없으니 이 알림은 무시하세요.) ` +
