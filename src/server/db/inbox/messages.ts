...
export function insertMessage(
  ...
  // ★★서버는 팀원이 쓴 주소를 고치지 않는다. 기록만 한다.★★ (OWNER 2026-07-14)
  //
  //   ★OWNER:★ "보정을 하면 안된다니깐.. 근본이 아니잖아. 그냥 기록만 추가하고 삭제해.
  ...
  //
  //   ★서버는 추측하지 않는다. 모델이 말한 사실만 쓴다.★ (OWNER 2026-07-14)
  //
  ...
  if (env.to_agent_id === "broadcast" || env.type === "broadcast") {
    ...
    // broadcast complete 로직 (OWNER 2026-06-22): broadcast(@all/announce)는 FYI다 — 비응답자도 inbox에
    ...
    if (!RESERVED.has(env.to_agent_id)) {
      ...
      if (agentExists) {
        ...
        // ★direct_to_gd 보고는 ingress [B] 릴레이(inbox.ts:275)가 이미 OWNER DM 에 배달한다★ → 수신자(to_agent_id)를
...
export function inboxFor(db: Database, agent_id: string, limit = 50): EnvelopeStored[] {
  ...
  // 받는이-단일화 (2026-06-13 OWNER 데이터모델 정리): inbox 멤버십은 message_recipient(받는이 테이블)
...
export function markRead(db: Database, message_id: string, agent_id?: string): boolean {
  ...
  // 받는이-단일화 (2026-06-13 OWNER): agent 의 읽음은 message_recipient(받는이 행)에만 기록한다.
 ...
 * (member 경로 A의 reply-blindness 보완). 아직 적재 전이면 null(호출측이 재시도/폴백).
 */
