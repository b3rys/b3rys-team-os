...
function sourceBoost(sourceType: SearchSourceType, query: string): number {
  ...
  const activeTaskLookup = /(team search|v0\.5|owner\s+(bill|codex|devon|demis|steve|dbak|hermes|brief|lui)|담당.*(누구|bill|codex|devon|demis|steve|빌|코덱스|데본|데미스|스티브))/iu.test(q);
  if (opsLike) {
