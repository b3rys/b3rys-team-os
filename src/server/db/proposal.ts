...
const VALID_TRANSITIONS: Record<string, string[]> = {
  ...
  // 팀장 결정은 승인/반려 둘뿐(OWNER 2026-07-10). 수정요청은 팀장 단계에서 제거 — 고칠 게 있으면
...
// member 같은 비운영 멤버가 peer로 배정되면 wake allowlist에서 만료되어 proposal이 stuck된다.
...
function eligiblePeerReviewCount(db: Database, proposalId: string, proposer: string): number {
  const skip = reviewSkipIds();
  const rows = db.prepare(
    `SELECT DISTINCT reviewer_agent AS id
      FROM proposal_review
      WHERE proposal_id = ?
        AND stage = 'peer'
        AND reviewer_agent != ?`,
  ).all(proposalId, proposer) as { id: string }[];
  return rows.filter((r) => !skip.has(r.id)).length;
}
...
/** 리뷰 추가(peer/pm/owner) + decision_log. is_adversarial=반대(거수기 방지). */
export function addReview(db: Database, r: { proposal_id: string; reviewer_agent: string; stage: string; verdict?: string; is_adversarial?: boolean; comments?: string; required_changes?: string }): { ok: boolean; id?: string; error?: string } {
  if (!["peer", "pm", "owner"].includes(r.stage)) return { ok: false, error: "stage must be peer/pm/owner" };
  const proposal = db.prepare("SELECT proposer_agent FROM proposal WHERE id = ?").get(r.proposal_id) as { proposer_agent: string } | undefined;
