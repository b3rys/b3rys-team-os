// GD 1:1 DM "누가 나한테 보냈나" 조회 — 비-tmux 런타임(openclaw/hermes) 챗 답변용.
// BWF 분담(Steve 킥오프): 사적DM분=Demis. (버스 message_recipient "누가 확인했나"=Ames)
//
// dm_message(direction="in" = GD→봇) 읽기 전용. ★member_id 격리 준수★ — 타 멤버의 GD DM은
// 절대 포함하지 않는다(dmCapture 프라이버시 원칙 그대로). 이 테이블은 GD 1:1 DM 전용이므로
// inbound 발신자는 항상 GD.
...
/**
 * 특정 멤버가 GD로부터 받은(inbound) 최근 DM (최신 → 과거). ★member_id 격리★.
 * direction='in'만 — 봇이 GD에게 보낸 out은 "누가 보냈나"에 해당 없음.
 */
...
export interface InboundDmSummary {
  memberId: string;
  sender: string; // GD 1:1 DM 전용 테이블이라 항상 "GD"
...
export function describeInboundDms(db: Database, memberId: string, opts: InboundDmOpts = {}): string {
  ...
  if (s.count === 0) return `${window} GD가 보낸 1:1 DM은 없습니다.`;
  const head = `${window} GD가 1:1 DM ${s.count}건을 보냈습니다. 최신: "${s.latest!.preview}" (${s.latest!.atKst}).`;
