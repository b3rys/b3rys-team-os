// OWNER와의 1:1 DM 캡처 — 재시작 후에도 recall이 보이도록 team.db에 durable 저장.
// ★버스 message 테이블과 완전 분리★: dispatch/wake 컬럼이 아예 없어 이중응대(재전달) 함정이
// 구조적으로 불가. recall만 이 테이블을 읽는다. member_id 필터로 멤버별 격리(타 멤버의 OWNER DM
// 열람 불가 — OWNER 개인데이터 프라이버시 built-in).
import type { Database } from "bun:sqlite";
import { nanoid } from "nanoid";

export type DmDirection = "in" | "out"; // in=OWNER→봇, out=봇→OWNER
...
/**
 * OWNER 1:1 DM 한 건 저장. dedupe_key UNIQUE + INSERT OR IGNORE로 재삽입 시 중복 무시
...
/**
 * 특정 멤버의 OWNER 1:1 최근 메시지 (최신 → 과거). ★member_id 필터 = 멤버별 격리★:
 * 타 멤버의 OWNER DM은 절대 포함되지 않는다. recall 슬롯에서 시간순으로 쓰려면 호출부에서 reverse.
 */
