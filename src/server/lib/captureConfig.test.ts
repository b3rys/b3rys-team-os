...
describe("captureConfig — 상태 마스킹(★토큰 값 노출 금지)", () => {
  test("captureConfigStatus는 토큰 값을 절대 포함하지 않음", () => {
    ...
    expect(status).toEqual({ has_capture_token: true, capture_group_id: "-100g", router_enabled: true, mcp_enabled: true });
    // 직렬화에도 토큰 값이 없어야 함
    expect(JSON.stringify(status)).not.toContain("SECRET-must-not-leak");
  });
});

// ── ★끄는 스위치★ (팀 리드 2026-08-07: "그냥 기본을 켜기로 해") ──
//
// 처음엔 기본을 꺼짐으로 뒀다. 그러니 ★쓰던 사람이 배포 순간 끊기지 않게★ 구제 마이그레이션이 필요했고,
// 그마저도 ★기록이 안 남는 읽기 전용 사용은 못 살렸다.★ 기본을 켜면 그 장치가 통째로 없어진다.
// 대신 ★"쓰기 전에 켜야 한다" 는 조건도 없어진다★ — 남는 건 끄는 스위치다.

test("★손댄 적 없으면 켜져 있다★ — 새 설치도, 쓰던 설치도", () => {
  const d = new Database(":memory:");
  migrate(d);
  expect(isMcpEnabled(d)).toBe(true);
});

test("★사람이 끈 것은 재부팅해도 안 되켜진다★", () => {
  const d = new Database(":memory:");
  migrate(d);
  setMcpEnabled(d, false);
  migrate(d); // 재부팅
  expect(isMcpEnabled(d)).toBe(false);
});

test("★껐다 켤 수 있다★ — 끄는 스위치는 되살릴 수 있어야 한다", () => {
  const d = new Database(":memory:");
  migrate(d);
  setMcpEnabled(d, false);
  expect(isMcpEnabled(d)).toBe(false);
  setMcpEnabled(d, true);
  expect(isMcpEnabled(d)).toBe(true);
});
