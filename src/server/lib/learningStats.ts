...
export function computeLearningStats(days = 1): LearningStats {
  ...
  routing.askOwnerRate = routing.total ? Math.round(((routing.byOutcome["ask_owner"] ?? 0) / routing.total) * 100) : 0;
