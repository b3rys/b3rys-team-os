// 봇 토큰 교체 — 런타임별 격리 핸들러 + fail-safe (OWNER 2026-07-01).
// 원칙(OWNER): getMe로 새 토큰 검증 → 기존 백업 → 쓰기 → 재시작 → 어디서든 실패하면 기존 복원+멈춤(절대 반쯤 안 바꿈).
...
function writeEnvKey(file: string, key: string, value: string, mode = 0o600): void {
  ...
  // atomic(temp+rename) — truncate-in-place 빈 창에 poller가 읽으면 토큰로드 실패로 죽음(하네스 근본원인). OWNER 2026-07-01.
  const tmp = `${file}.tmp`;
...
// ── 단독 파일(member var/secrets, openclaw token.txt) ──
...
export function resolveTokenStore(runtime: string, id: string, agent: AgentRecord): TokenStore | { unsupported: string } {
  ...
  if (runtime === "openclaw") {
    ...
    // ★파일기반 계정만 rotate 허용(OWNER 2026-07-05)★: openclaw.json 에 tokenFile 이 정의된 계정(lui·devon 등)만.
