...
interface InjectOpenclawTelegramOptions {
  ...
  hopCount?: number;
  /** case 6 (2026-06-05): Bill 등이 위임한 OWNER-facing 보고(direct_to_gd). 프롬프트에 "OWNER에게 직접
...
// 2026-07-06 (OWNER live): 300초는 코드 수정·검증 같은 긴 turn에서 bridge가 먼저 포기해
...
// 2026-06-05 롤백(OWNER): 오늘 넣었던 "작성 중"(EARLY_PROGRESS)·별도 보이는-한도(VISIBLE_REPLY_TIMEOUT)
...
function resolveOpenclawAgentId(agent: AgentRecord): string {
  // 게이트웨이 프로필명은 agents.json 의 openclaw_agent_id 정본을 쓴다(codex="owner" 등 이미 박힘).
  // 이전 `agent.id === "codex" → "owner"` 하드코딩 폴백은 codex 레코드가 openclaw_agent_id 를 이미
...
function openclawTelegramBotToken(agent: AgentRecord): string | null {
  ...
  // native_routing agent(codex)만 공유 openclaw env 파일(owner 프로필 토큰)을 fallback 으로 읽는다.
  ...
  // 못 올리던 버그(OWNER 2501) 수정. 값은 런타임에 파일에서 읽고 어디에도 복사하지 않는다.
  try {
...
export async function runOpenclawSlackTurn(opts: RunOpenclawTurnOptions): Promise<string> {
  ...
  const prompt =
    ...
    `${opts.body}\n` +
    `</external_message>\n\n` +
    // ★[B] — 말하려면 보내라.★ (OWNER 2026-07-13) 예전엔 "직접 보내지 마세요, 브릿지가 전송합니다" 였다.
    // 보내는 법(send.sh)은 룰에 있다 — 여기서 두 번째 입구를 열지 않는다(OWNER 2026-07-14).
    pick(opts.locale,
 ...
 * (devon handoff 미응답 버그, OWNER 2489). claude 봇이 directed 도 tmux 로 깨워지는 것과 동일하게 맞춘다.
 */
export async function injectOpenclawDirectedTurn(opts: InjectOpenclawDirectedOptions): Promise<boolean> {
  ...
  // ★sessions.send(fire-and-forget) → agent + agent.wait★ (2026-07-16, OWNER)
...
export async function injectOpenclawTelegramTurn(opts: InjectOpenclawTelegramOptions): Promise<boolean> {
  ...
  const hopInstruction = opts.inReplyTo !== undefined
    ...
    : "";
  // 사실만: 이 보고는 ${owner} 가 직접 볼 것이다 → --direct-to-owner. 보내는 법의 상세는 룰에 있다.
  const directReportNote = opts.directReport
    ? pick(locale,
        `[direct_to_gd] 이 작업은 ${opts.fromLabel ?? "a teammate"}이(가) 위임한 ${owner}-facing 보고입니다 — ${owner}가 결과를 직접 보길 원합니다. 최종 보고는 ★--direct-to-owner 로 보내세요★ (서버가 ${owner}의 1:1 DM 으로 릴레이합니다). 안 보내면 아무 데도 안 갑니다.\n\n`,
        `[direct_to_gd] This task is an ${owner}-facing report delegated by ${opts.fromLabel ?? "a teammate"} — ${owner} wants to see the result directly. **Send your final report with --direct-to-owner** (the server relays it to ${owner}'s 1:1 DM). If you do not send, it goes nowhere.\n\n`)
    : "";
  const prompt =
    teamContextBlock +
    attachmentBlock +
    ...
    `${opts.body}\n` +
    `</external_message>\n\n` +
    // ★사실만 말한다(OWNER 2026-07-14).★ 예전 문구는 envelope API(두 번째 입구)를 두 번 안내하고,
  ...
  // 2026-07-06 (OWNER live): timeout 뒤 침묵하면 팀장 입장에서는 "아무것도 안 함"이 된다.
  ...
  //   ★"OWNER Step Codex: [NO_REPLY]" 가 팀장 단톡방에 문자 그대로 찍혔다.★ (2026-07-13 라이브)
  //
  // ═══ 지금 ═══
