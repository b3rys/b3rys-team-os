// persona 쓰기 단일 canonical 경로 (OWNER 아키텍처 지시, Bill 핸드오프 2026-07-05).
//
...
//   - backup-first: 덮기 전 기존 파일 `.bak` (OWNER 데이터 무조건 백업 원칙).
...
/**
 * ★워크스페이스에 TEAM-OS.md 심링크를 보장한다★ (OWNER 2026-07-13 — "team-os 심링크는 영입때도 체크")
 *
 * ═══ 왜 (member가 잡았다) ═══
 ...
 */
/**
 * ★persona 값을 저장하는 유일한 지점 — SOUL.md 에만 쓴다.★ (OWNER 2026-07-17)
 *
 *   "persona 값은 그냥 soul.md 에만 저장해. 대시보드 나머지 필드는 agents.json이 원본이면 되고" — OWNER
 *
...
export function writeMemberPersona(m: WriteMemberPersonaInput): WriteMemberPersonaResult {
  const workspace = m.workspace_path ?? memberPaths(m.id, m.runtime).workspace_path;
  assertNotLiveMemberFsUnderTest(workspace, `writeMemberPersona(${m.id})`); // FIX2(OWNER 2026-07-08): 테스트가 라이브 persona 덮어쓰기 못 하게 차단
  const targets = personaTargetsForRuntime(m.runtime, workspace, m.persona_file);
  ...
  const put = (file: string, content: string): void => {
    // skip-if-unchanged: 렌더 결과가 기존 파일과 동일하면 write·bak 생략 (OWNER 2026-07-19 — 룰 변화 없으면 매 부팅 재작성하지 마라).
  ...
  ensureTeamOsLink(workspace);   // ★영입 때부터 팀 룰 정본을 읽을 수 있게★ (OWNER 2026-07-13)
