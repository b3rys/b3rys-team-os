...
export interface HermesTurnOptions {
  ...
  fromLabel: string;
  /**
   * ★답이 어디로 가야 하는지 — 부르는 쪽이 ★안다★. 주입문이 추측하면 안 된다.★ (OWNER 2026-07-14)
   *
   * 예전엔 주입문 꼬리가 무조건 "★팀장께★ 답하라" 였다. 그런데 이 주입문은 ★4가지 상황★ 에 쓰인다:
   *   버스 1:1(member가 물음) / 단톡방(팀장이 부름) / 팀장 직보 / 슬랙 — ★정답이 전부 다르다.★
  ...
  replyRoute:
    ...
    | { kind: "direct_to_gd" } // 팀장 직보 (--direct-to-owner)
...
export function buildPrompt(opts: HermesTurnOptions): string {
  ...
  // Hermes 전용 함수호출 강제(OWNER 2026-07-09): hermes 런타임이 공유 persona 룰만으론 맞장구 루프를
  ...
  //   빗나가 ★재-wake → OWNER 중복보고★ 위험. 실제 id 를 문자열로 주면 모델이 복사만 하면 되어 결정론적.
  //
  ...
  // ★★답은 "보낸 사람" 에게 간다. 팀장께가 아니다.★★ (OWNER 2026-07-14: "팀원이 왜 잘못 보냈는지 찾아야지")
  //   ★이 줄이 hermes 가 방에 대고 답하던 진짜 원인이었다.★
  //   예전엔 `${owner}께 답하고` = ★"팀장께 답하라"★ 였다. 그런데 이건 ★팀버스 메시지★ 다 —
  //   보낸 사람은 member인데 "팀장께 답하라" 고 시켰다. 그래서 hermes 는 방(broadcast)에 대고 썼다.
  ...
  const context = opts.teamContext
    ...
    : "";
  // (2026-07-10 제거, OWNER 결정): directReportNote(자가발송 금지 문구)는 hermes 자가발송을 막으려 넣었으나,
  ...
  //   (directReportNote 의 'DM·그룹금지'와 상충 방지, OWNER 2026-07-09). 일반 턴은 그룹 표시 안내 유지.
  // ★[B] — 말하려면 보내라.★ (OWNER 2026-07-13: "팀원한테 맡겨. 다 빼.")
  ...
  // ★주소 메뉴를 주지 않는다.★ (OWNER 2026-07-14 / hermes 본인 확인)
  ...
  // ★from=system 이면 그게 무슨 뜻인지 ★말해준다.★★ (OWNER 2026-07-14)
 ...
 *   여기 하나만 바꾸면 사다리(turnCap<lease<grace)가 따라온다 — 빼먹을 일 없음 (OWNER 2026-07-16).
 */
...
export async function runHermesTeamTurn(opts: HermesTurnOptions): Promise<string> {
  ...
  // ★턴 상한은 ★한 곳★ 에서만 정한다.★ (OWNER 2026-07-15: "턴 상한도 한 군데서 수정하게")
  ...
  //   (60s 는 가벼운 답엔 충분하나 수집·리서치·종합형 team-turn 은 초과 → dead_letter. OWNER 2026-07-09 라이브
  ...
  return new Promise((resolve, reject) => {
    const proc = spawn(cmd, ["-z", prompt, "--usage-file", usagePath], {
      ...
      env: {
        ...process.env,
        OWNER_AGENT_ID: opts.agent.id,          // 나는 누구인가 (스크립트가 tmux 세션으로 추측하지 않게)
...
// ★hermes 텍스트 전송 = Bot API sendMessage 직접★ (2026-07-15, OWNER 지시)
//   예전엔 `hermes send` CLI 를 spawn 했다 → cold 기동이 15초 타임아웃을 넘기면 error 없이 false
//   (2026-07-15 라이브: owner-report hermes 1건 유실, "unknown"). 리액션(reactTelegramAsHermes)은 이미
