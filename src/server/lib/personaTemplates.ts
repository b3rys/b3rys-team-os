...
const CORE_RULE_COMPACT = [
  ...
  "- **Abbreviations**: expand once in parentheses on first use — `RoPE(Rotary Position Embedding)` — then keep the original name, not a Korean paraphrase.",
  "- **No analogy, no literal translation.** Exception: an analogy already in the source stays, flagged as the source's wording. Source meaning wins.",
  "- **Volume**: do not pad. Stay on the situation, the facts, the terms, and the example. No intensifiers or set phrases. Write the number instead of 많다 · 대부분 — `17개 중 16개`.",
  "- **When corrected, fix that spot.** Swapping in another coined word repeats the defect under a new name (`동작 안하고 있었습니다` → `아무 일도 하지 않았습니다`; `권한 열쇠` → `검문소`). Ask what the real name is first.",
  "- **Comments and docs follow the same rule.** A coined word in a comment stays in the file after the conversation ends, and the next reader cannot search it. Rewrite it as what the code does.",
  "",
  "**Safety·verification**",
