...
export function ruleLoadingBlock(runtime: string, agentId?: string): string {
  ...
  return [
    ...
    ...(isOpenclaw
      ? ["- **Skill creation uses the b3os way by default (NOT OpenClaw's own Skill Workshop)**: improvements/proposals go through a **b3os proposal** (`prop_...`); actual tools/skills are built in the **b3os skill system** (`b3os-<area>-<function>` under `team-collab/skills`, catalog `docs/B3OS_SKILLS.md`). OpenClaw's `skill_workshop` (apply/reject/quarantine) is only for real Skill Workshop proposals (do not confuse it with b3os `prop_...`)."]
      : []),
...
function sectionTeamShare(runtime: string, agentId?: string): string {
  if (runtime === "claude_channel") {
    return [
      ...
      "- **b3os-team-inbox skill** — the team message-bus tool (inbox·send·ack). Use it when messaging teammates. (canonical = team-collab/skills; it is shell, so all runtimes use it)",
  ...
  return [
    ...
    "- Team skills = `team-collab/skills` (canonical) + catalog `docs/B3OS_SKILLS.md`. Run shell skills via `scripts/*.sh` directly. (openclaw has no skill auto-discovery — find it in the catalog and read the `SKILL.md` directly to follow it.)",
    "- **★When sending a message/reply/review-request to a teammate, you MUST use `team-collab/skills/b3os-team-inbox/scripts/send.sh --to <them> --body \"…\"`. Do NOT try to send via OpenClaw's sessions_* / dynamic session routing (the agentId isn't resolvable in this runtime, so it fails).** Check what you received with the same skill's `inbox.sh`. If a path is unclear, don't guess — find b3os-team-inbox in `docs/B3OS_SKILLS.md` and read its `SKILL.md` first.",
    "- For sending a Telegram file (HTML·PDF·image·ZIP·document), use the `b3os-telegram-file-delivery` skill (Bot API sendDocument) by default — use this canonical procedure instead of the message/attachment tools that get blocked.",
