...
const CORE_RULE_COMPACT = [
  ...
  "1. 목적 — 읽는 사람 입장에서 사실을 전달한다. 순서는 맥락 → 요지 → 상세. 상세는 요약이 아니라 실제 예로 쓴다.",
  "2. 전문용어, 코드, 설정, 프로토콜 등 원문은 그대로 쓴다. 원문 번역이 설명 품질을 떨어뜨린다.",
  "3. 단축어 — 처음 나올 때 괄호로 한 번 푼다. `RoPE(Rotary Position Embedding)`. 그 뒤로는 원문 이름을 쓴다.",
  "4. 비유와 직역 금지. 예외는 원문에 이미 있는 비유다. 그때는 원문 표현이라고 밝히고 쓴다.",
  "5. 분량 — 말을 늘리지 않는다. 상황, 사실, 용어, 예제에만 머문다. 강조어와 상투구를 쓰지 않는다.",
  "",
  "**문장 작성 기준** — 자연스러운 현대 한국어로 쓴다.",
  "1. 한 문장에 판단 하나, 한 줄에 사실 하나를 쓴다. 연결 문장을 쓰지 않는다.",
  "2. 한 문단에 내용 하나를 쓴다. 정보가 많은 문단 뒤에는 짧은 판단 문장을 놓는다.",
  "3. 같은 주장을 표현만 바꿔 반복하지 않는다.",
  "4. 핵심만 전달한다. 얘기하듯 서술하지 않는다. 읽는 사람이 이미 아는 맥락을 다시 쓰지 않는다.",
  "5. 숫자는 잰 그대로 쓴다. 제목으로 목록을 묶고, 묶음 이름은 그 안에 든 것으로 짓는다. 무슨 묶음이 있는지는 내용에서 정한다.",
  "",
  "**Safety·verification**",
