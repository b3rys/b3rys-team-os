...
const CORE_RULE_COMPACT = [
  ...
  "",
  "**Explaining** — ⭐ the team lead calls these the 설명 원칙 (explanation principles). They apply to every message, report, PR body, **and code comment**.",
  "- **Purpose**: deliver the facts from the reader's side. Order: context → the point → the detail, and give the detail as a real case (situation, steps, what changed) rather than as a summary. Lead with context and the point so the reader has the whole shape before the parts, not where your own work left off.",
  "- **Names**: if code, config, or a protocol already has a name, use that name verbatim — `window.scrollTo(x, y)`, `Cache-Control: no-store`, `approval key`. Never translate it and never coin a Korean common noun for it — `BridgeWindow` became `창구` in that file's own comments, `hash` became `지문`, `key` became `이름표`. Ordinary dev terms (server, endpoint, cache) stay as-is.",
  "- **★When there is no name, do not invent one — write what happens.★** This is the half that is missed most: `실물`·`소유자`·`기계`·`제자리` were coined for things that have no such name, so the rule above never fires. **Test: name the variable, field, or function that noun stands for. If you cannot, you invented it** — grepping the repo does not work, because a coined word already sitting in old comments will match. Then describe the action instead: `실물은 전부 소유자를 하나로 못박고` → `에디터들은 본문을 두 곳에 저장하고, 어느 쪽을 기준으로 삼을지 하나로 정한다`.",
  "- **Abbreviations**: expand once in parentheses on first use — `RoPE(Rotary Position Embedding)`. **After that keep using the original name, not a Korean paraphrase.** Explaining once does not license switching to a coined word for the rest of the text.",
  "- **No analogy, no literal translation**: an abstract comparison or a word-for-word Korean rendering replaces the fact instead of delivering it. Exception: an analogy already in the source stays, flagged as the source's wording. Source meaning wins.",
  "- **Volume**: do not pad. Stay on the situation, the facts, the terms, and the example. No intensifiers or set phrases (in effect, ultimately, practically, so to speak). **Write the number instead of words like 많다 · 대부분 · 크게 · 상당히** — `17개 중 16개` beats `대부분`.",
  "- **When corrected, fix that spot — do not swap in another word.** Replacing a flagged phrase with a different coined one repeats the defect under a new name (observed: `동작 안하고 있었습니다` → `아무 일도 하지 않았습니다`; `권한 열쇠` → `검문소`). Ask what the real name is first.",
  "- **Comments and docs follow the same rule.** A coined word in a code comment outlives the conversation: the next reader opens the file and it calls itself by a name that is not in the code and cannot be searched. Found an abstract term in a comment → rewrite it as what the code actually does.",
  "",
  "**Safety·verification**",
