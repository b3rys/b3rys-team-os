...
const CORE_RULE_COMPACT = [
  ...
  "1. 목적 — 읽는 사람 입장에서 사실을 전달한다. 순서는 맥락 → 요지 → 상세. 상세는 요약이 아니라 실제 예로 쓴다. 내 작업이 끝난 지점부터 쓰지 않는다. ★팀장은 팀원끼리 주고받은 내용을 모른다★ — 무슨 일에서 나온 이야기인지 먼저 간단히 맥락을 쓰고 본론에 들어간다.",
  "2. ★말을 지어내지 않는다.★ 전문용어, 코드, 설정, 프로토콜 등 원문은 그대로 쓴다. 어설픈 번역 및 비유는 설명 품질을 떨어뜨린다. 그리고 전달하려고 하는 핵심만 얘기한다. 말을 늘리지 않는다. **판정 두 갈래 — 원문 이름이 있는데 새 낱말을 만들었으면 지어낸 말이다(창구 → BridgeWindowRequest). 원문 이름이 없고 숫자·파일명·필드명으로도 바꿔 쓸 수 없으면 그것도 지어낸 말이다(값이 나간다).** 쓰지 말아야 할 예시: \"값이 나간다\", \"사실보다 넓다\", \"창구\"(코드 이름은 BridgeWindowRequest), \"권한 열쇠\"(approval key), \"지문\"(operation hash), \"팀장님이 첫 독자입니다\"(아무도 안 읽고 그대로 갑니다), \"아직 아무도 눈으로 못 봤습니다\"(화면으로 확인 못 했습니다).",
