...
/** openclaw 자체 auth store 존재 확인 — ~/.openclaw/agents/<agent>/agent/auth-profiles.json 이
 *  하나라도 있으면 openclaw 가 인증돼 있고 activate-openclaw 의 auth 복제 소스가 있는 것.
...
// plain codex = ~/.codex/auth.json 존재로 빠르게 점검. openclaw는 자체 auth store(auth-profiles.json)로
...
// openclaw 는 자체 auth store(auth-profiles.json)로 인증한다 — codex login 이 아님.
const RUNTIME_SETUP_DOC = "절차: skills/b3os/references/runtime-setup.md";
const OPENCLAW_FIX_HINT = `먼저 openclaw 에이전트를 하나 인증하세요(auth 복제 소스 필요). 이 서버 컴퓨터에서 \`npm install -g openclaw@latest\` → \`openclaw onboard --install-daemon\` → \`openclaw doctor\` 완료 후 다시 활성화하세요. ${RUNTIME_SETUP_DOC}#openclaw`;
...
async function checkCodexAuth(runtime: string): Promise<RuntimeAuthResult> {
  ...
  if (runtime === "openclaw") {
    ...
    // ② openclaw 로그인 여부 = 자체 auth store(~/.openclaw/agents/*/agent/auth-profiles.json) 존재.
    ...
    return {
      ...
      loggedIn,
      detail: loggedIn
        ? "openclaw 인증 확인됨(auth-profiles.json)"
        : "openclaw 미인증(~/.openclaw/agents/*/agent/auth-profiles.json 없음 — openclaw 에이전트를 하나 인증하세요)",
...
const CODEX_FAMILY = new Set(["codex", "openclaw"]); // checkCodexAuth 로 분기(같은 바이너리군). 단 인증 소스는 다름: plain codex=~/.codex/auth.json, openclaw=자체 auth-profiles.json. hermes는 분리(checkHermesAuth).
