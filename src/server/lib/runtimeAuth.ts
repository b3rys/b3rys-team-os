// 런타임 인증 preflight (GD 2026-06-29): 영입 활성화 전에 claude/codex 로그인 상태를 사전점검한다.
...
import { openclawCliCandidates } from "./openclawBridge";
 ...
 *  ~/.codex/auth.json(GD 머신 심링크)에 의존하면 공개 openclaw 사용자를 오차단하므로 별도 판정.
 *  값은 절대 읽지 않고 파일 존재만 확인. (하네스 fix, GD 2026-07-02) */
function openclawAuthExists(): boolean {
  try {
    ...
    //   per-agent 만 보던 기존 탐지가 '로그인했는데 미로그인'으로 오차단했다(BUG8 버전드리프트, GD 2026-07-03).
 ...
 *  값은 절대 읽지 않고 파일 존재만 확인(openclawAuthExists 패턴 재사용). (하네스 fix, GD 2026-07-02) */
...
export interface RuntimeAuthResult {
  ...
  fixHint: string; // 미로그인 시 GD 안내 문구
}
...
// 바이너리 자체가 없을 때(homebrew cleanup/업데이트로 사라짐 등) — '미로그인'과 구분해 명확히 안내(GD 2026-07-01).
...
/** 바이너리 존재 확인 — Bun.which(PATH) ∪ 흔한 경로. launchd 제한 PATH서도 잡히게 고정경로 병행. 미설치 vs 미로그인 구분용(하네스 HIGH, GD 2026-07-02). */
 ...
 *  나중에 불투명한 first-model-call exit_null 만 남긴다(BUG6, GD 2026-07-03). 값/출력은 안 봄(stdio ignore). */
...
function checkClaudeAuth(): RuntimeAuthResult {
  ...
  //     타임아웃(오해 유발)으로만 나타난다. 여기서 미리 명확히 안내(하네스 fix, GD 2026-07-02).
  if (!binaryExists("tmux", process.env.TMUX_BIN ?? "/opt/homebrew/bin/tmux", "/usr/local/bin/tmux", "/usr/bin/tmux")) {
    return { ...base, loggedIn: false, detail: "tmux 미설치(claude_channel 주입에 필요)", fixHint: "tmux가 필요합니다: 이 서버 컴퓨터 터미널에서 `brew install tmux`(또는 OS 패키지 매니저)로 설치 후 다시 활성화하세요. Homebrew 자체가 없으면 https://brew.sh 로 먼저 설치하거나, repo 폴더에서 `bash install.sh` 를 다시 실행하면 tmux 까지 자동 준비됩니다" };
  }
...
/** hermes_agent: 바이너리 존재(미설치 구분) → 자체 auth(~/.hermes/auth.json). codex와 별개 백엔드라 ~/.codex 프록시 판정 금지(하네스). 값 미열람. GD 2026-07-01. */
function checkHermesAuth(): RuntimeAuthResult {
  ...
  // 활성화가 조용히 실패. openclaw 분기와 동일하게 사전 차단(하네스 fix, GD 2026-07-02).
  ...
  // (프로필만 있는 사용자 오차단 / 글로벌만 보고 통과 후 exit 1 하는 divergence 제거, 하네스 fix, GD 2026-07-02).
...
async function checkCodexAuth(runtime: string): Promise<RuntimeAuthResult> {
  ...
  if (runtime === "openclaw") {
    // ★점검과 실행이 같은 해석을 쓴다★ — 이게 어긋나서 "설치됨(초록)인데 깨우기는 실패" 가 났다.
    //   ★후보 목록만 맞추면 부족하다★ (2026-07-27 Codex 리뷰 적발): 실행부 resolveOpenclawBin() 은
    //   OPENCLAW_BIN 을 ★최우선★ 으로 존중하는데, 점검이 후보 목록만 보면 비표준 prefix 에 설치하고
    //   OPENCLAW_BIN 을 지정한 ★정상 지원 환경★ 을 "CLI 미설치" 로 오차단한다. 이번엔 방향만 반대일 뿐
    //   같은 부류의 어긋남이다. 그래서 실행부가 고른 값 자체를 후보 맨 앞에 넣는다.
    if (!binaryExists("openclaw", ...openclawCliCandidates())) {
    ...
    //    ~/.codex/auth.json 은 GD 머신 심링크라 공개 openclaw 사용자엔 없음 → 그걸로 판정하면 오차단.
    //    openclaw 는 codex login 을 안 거치고 자체 인증하므로 openclaw 소스로만 판정(하네스 fix, GD 2026-07-02).
  ...
  if (!binaryExists("codex", process.env.CODEX_BIN ?? CODEX_BIN, "/opt/homebrew/bin/codex", "/usr/local/bin/codex", `${HOME}/.local/bin/codex`)) {
    // 아키텍처/OS 무관 감지(하네스 HIGH, GD 2026-07-02): Bun.which(PATH) ∪ Apple-Silicon(/opt/homebrew) ∪ Intel(/usr/local) ∪ ~/.local. 고정경로 하나로 판정하면 Intel/Linux서 정상 codex를 '미설치'로 오판→활성화 차단.
    return { ...base, loggedIn: false, detail: "codex CLI 미설치(바이너리 없음 — brew install codex 필요)", fixHint: CODEX_MISSING_HINT };
  }
  // ①-b 바이너리 실행 검증(BUG6, GD 2026-07-03) — 존재하지만 실행 불가(macOS XProtect 격리·OpenAI 인증서 폐기)면
  ...
  //    preflight 타임아웃(8s)에 걸려 false '미로그인' → codex 계열 영입이 preflight서 막히던 근본버그(GD 2026-07-01).
