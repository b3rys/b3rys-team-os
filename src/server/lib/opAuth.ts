...
export function leadActorId(db?: Database): string {
  return leadActorSetting(db ?? leadActorDb) ?? process.env.LEAD_ACTOR_ID ?? "owner";
}
...
export function fallbackLeadActorId(): string {
  return process.env.LEAD_ACTOR_ID ?? "owner";
}
...
export function trustedActorFromHeaders(headers: Headers): AuthResult {
  ...
  if (!tokenMatches(headers.get("x-op-token") ?? undefined, expected)) {
