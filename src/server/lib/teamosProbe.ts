 ...
 *
 * Design constraint (OWNER): this must NOT touch the team bus's own function or
...
function listLaunchd(running: Map<string, boolean>): TeamOsScheduled[] {
  ...
  return files.sort().map((f) => {
    ...
    //   to the Agent cards for real liveness. (OWNER 2026-05-31: bots were all false-red.)
...
function listScheduledJobs(db: Database): TeamOsScheduled[] {
  try {
    ...
    return rows.map((r) => {
      ...
      const last = r.last_run_at ? `last=${r.last_run_at}` : "last=-";
      return {
        label: r.id,
        kind: "scheduled" as const,
        detail: [cron && `cron=${cron}`, `next=${r.next_run_at ?? "-"}`, last].filter(Boolean).join(" · "),
        description: r.title ?? "",
