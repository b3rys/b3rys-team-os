...
// 라우팅용: 인용/예시 구간 제거 → 그 안 @멘션·@all 은 트리거하지 않는다 (OWNER 2026-06-25).
 ...
 * codex 의 경우 기존 CODEX_ALLOWED_MENTION 과 동치(@codex/@member+조사/@example_openclaw_bot).
 */
...
function hasAddressedAlias(text: string, alias: string): boolean {
  ...
  // Korean direct address: "빌, ...", "member야 ...".
  ...
  // Mid-sentence assignment: "게임은 member가 해보자". Avoid historical references like
  // "member이 전에 말한" by requiring an action verb shortly after the particle.
...
/**
 * OWNER 개인 컨벤션(2026-06-02, config로 분리 — 외부 공개 시 미설정이면 코어는 깨끗):
...
// 과거/상태/산출물 참조("member가 전에 말한", "member가 만든", "Demis 가 일어났네") 는 호출이 아니므로 제외.
...
// OWNER 결정(2026-05-24): 평문 이름은 언급/컨텍스트일 수 있으므로 wake 신호가 아니다.
...
// over-summon 방지용 (OWNER 2026-05-24): "대상은 빌/member/member/member/member이겠지" 같은 나열에서 다 깨우지 않게.
...
function filterLiveWakeTargets(text: string, agents: AgentRecord[], targets: string[]): string[] {
  // scope-list 가드 제거 (2026-05-25, OWNER 결정): @멘션 필수(detectAddressedNamesLoose) + 아래 REQUEST_MARKER
  ...
  return targets.filter((id) => {
    ...
    // Fix 2026-05-25: 여러 명 명시 @멘션("@member @member @member 들려?")시 lead 한 명만 깨우던 버그 →
