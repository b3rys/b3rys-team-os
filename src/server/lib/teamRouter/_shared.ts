...
export interface LlmRouteDecision extends RouteDecision {
  ...
  /**
   * 여러 단계(fan-out + 보고 회수) 위임이면 true. OWNER 지침(2026-05-24): 자동 실행하지 말고
   * 수신 에이전트가 OWNER 에게 계획을 재확인(컨펌)한 뒤 진행. 라우터는 신호만 올린다.
   */
  needsOwnerConfirm?: boolean;
  /**
   * 결정 종류 (OWNER 설계 2026-05-24):
   * - route: 정상 라우팅 (targetAgentIds 로 보냄)
   * - closure: 종료/그만 신호 → 아무도 안 깸 + sticky 해제 (targetAgentIds=[])
   * - ask_owner: 담당 애매 → OWNER 에게 "누가 볼까요?" 질문. suggested 는 LLM/키워드 추천 후보(결정 아님).
   * (routeTeamMessageLLM 순수 경로는 항상 route 의미라 생략 가능 — hybrid 는 항상 명시.)
   */
  outcome?: "route" | "closure" | "ask_owner";
  /** ask_owner 일 때 LLM/키워드가 제안하는 후보 (OWNER 가 확정/변경). 결정이 아니라 추천. */
  suggested?: string[];
}
