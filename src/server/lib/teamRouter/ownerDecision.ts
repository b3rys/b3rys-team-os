...
export function routeTeamMessage(
  ...
  // 인용/예시(코드펜스·"—-" 구분선 아래)의 멘션은 트리거 대상에서 제외 (OWNER 2026-06-25).
  ...
  // @멘션 없음 + 답장이면 → 원문 작성자가 owner (sticky 보다 우선). OWNER 커뮤니케이션 룰.
  ...
  // 2026-06-05: topic_shift/closure 자동감지 제거(OWNER). sticky 는 명시적 @멘션/답장으로
...
// ─────────────────────────────────────────────────────────────────────────────
// LLM router (EXAONE via Ollama) — PRIMARY engine per OWNER's "all-LLM, no regex"
...
// execution intent (논의=multi / 구현=single owner) — OWNER pattern.
// ─────────────────────────────────────────────────────────────────────────────
...
function routerSystemPrompt(agents: AgentRecord[]): string {
  const coord = coordinatorId(agents) ?? "코디네이터"; // 기본 조율 담당 = capability 기반(agents.json), 특정 이름 하드코딩 금지(OWNER 2026-07-02)
...
규칙(우선순위 순):
1. @멘션으로 특정인을 부르면 → 그 사람(들). 최우선. Codex는 예외적으로 정확히 @member, @codex, @Codex, @example_openclaw_bot 일 때만 호출로 본다. 문장 중간의 "member" 언급은 호출이 아니다.
 ...
 * standalone — 아직 라이브 메시지 흐름에 연결 안 됨 (OWNER 리뷰 후 통합).
 */
...
// (OWNER 의 "all-LLM" 결정과의 트레이드오프: 신뢰도 위해 명확신호는 결정론. OWNER 리뷰 후 택1.)
// ─────────────────────────────────────────────────────────────────────────────
...
export async function routeTeamMessageHybrid(
  ...
  const activeAssigneeIds = validActiveAssignees(context, agents);
  // 인용/예시(코드펜스·"—-" 구분선 아래)의 멘션은 트리거 제외 (OWNER 2026-06-25). 라이브 멘션 판정에만 적용.
  const liveText = stripQuotedForRouting(text);

  // 2026-06-05: topic_shift/closure 자동감지 제거(OWNER). owner 는 @멘션 > 답장 > sticky > 기본
  ...
  // 1) 명시 @멘션 — @멘션은 어떤 상황에서도 우선 (OWNER 2026-05-25). detectExplicitTargets 는
  // 순수 @멘션/@별칭만 잡고(과거참조 tail drop·REQUEST_MARKER 필터 없음) 무조건 깨운다.
  // 직접 호출 "@member @member 이거 보여. 어제 모했어" 가 tail 의 '어제'(과거참조)+요청마커 부재로
  ...
  if (wakeTargets.length > 0) {
    // 2026-06-05 (OWNER): @멘션은 컴 룰의 최상위. 잡힌 전원에게 라우팅하고 끝낸다.
    // 위임/중계/보고/작업 판단은 멘션 받은 에이전트(LLM)가 내용을 읽고 한다 — 라우터는 'owner 가 누구냐'만.
    // (이전 analyzeDelegation 좁히기는 "@member @member 각자 일"을 위임으로 오판하는 등 버그만 양산해 제거.)
    return {
  ...
  // 1.5) @멘션 없음 + 답장(reply)이면 → 원문 작성자가 owner. OWNER 커뮤니케이션 룰:
  ...
  // 4) (OWNER 룰 2026-06-05) 오너가 애매하면(멘션·답장·sticky 모두 없음) codex 로 자동 배정하지 않는다.
  //    룰 = @멘션 > 답장 > sticky > **오너 애매하면 OWNER 문의**. → ask_owner(아무도 안 깨움 + OWNER 에게 누가 볼지 질문).
