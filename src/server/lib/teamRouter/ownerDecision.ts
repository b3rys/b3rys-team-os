...
/**
 * ★팀원 방 발언이 팀원 inbox 로 전달될 대상인가★ — `@all` 마커 하나만 본다.
 *
 * 방 게시와 팀원 전달이 `broadcast` 한 단어에 묶여 있어서 ★어디에 말하든 전원이 깨어났다.★
 * 갈라야 할 축은 "몇 명" 이 아니라 ★"애초에 전달이 필요한 발송인가"★ 다 (GD 2026-08-01).
 *
 * 실측(2026-08-01 방 발언 81건): ★멘션 없음 51건 → 답 달린 것 0건★ · 멘션 있음 30건 → 답 27건.
 * 잘라도 끊기는 대화가 없었다. (그 51건은 대부분 팀장님께 드리는 답이었다.)
 *
 * ★@이름 멘션은 여기서 안 본다.★ 팀원끼리는 팀버스(`--to <id>`)로 하고 방에서 부르지 않는다는 게
 * 팀 규칙이라, 방 발언의 예외는 `@all` 하나다. 멘션·답장·sticky 사다리는 ★팀장님 메시지에 답할
 * 담당을 가리는 규칙★ 이고 이 함수와 무관하다.
 *
 * ★마커 판정은 라우터 정본(`BROADCAST_MARKER_RE`)을 재사용한다★ — 여기서 정규식을 새로 쓰면
 * 그게 두 번째 판정이 되고, 그게 이 결함의 원인이었다.
 */
export function broadcastAudience(text: string): { kind: "all_hands" | "none" } {
  // `stripQuotedForRouting` 은 ★코드펜스·구분선 아래만★ 지운다. ★인용줄(`> …`)은 안 지운다★ —
  //   즉 팀장님 원문을 `> @all …` 로 인용해 답하면 ★그것도 전원 팬아웃이 된다.★ (실측 확인)
  //   main 도 같은 동작이라 이번 변경의 회귀는 아니지만, ★고칠 자리로 남아 있다.★
