...
3. 여러 담당이 가능하거나 확신이 낮으면 outcome=ask_gd, suggested=[후보 id]. 이 경우 시스템은 ${coord}에게 보낸다.
4. 순수 결정/승인/판단 요청이면 outcome=ask_gd. 이 경우 시스템은 ${coord}에게 보낸다.
4. 토큰/시크릿/삭제/배포/서비스 재시작/DB migration/외부 발신/비용 발생은 needs_gd_confirm=true.
...
JSON만 출력:
{"outcome":"route|ask_gd","responder":"<id|null>","suggested":["<id>",...],"domain":"<짧은 범주>","needs_gd_confirm":true|false,"reason":"<짧게>"}`;
}
