// 승인 큐 — OWNER 승인이 필요한 권한 액션을 모아 PIN 승인으로 실행(터미널 0). (2026-06-10 OWNER)
//
// 보안 모델:
...
export const ACTIONS: Record<string, ApprovalAction> = {
  ...
  merge_to_main: {
    ...
    description:
      ...
      "현재 승인 모델 = OWNER 탭 승인(승인자=OWNER, 작성자=에이전트 → self-approve 원천 불가). " +
...
export type ApprovalStatus =
  ...
  | "rejected"
  | "expired"
  | "deferred"; // 승인 시스템 v2(OWNER 2026-07-08): 10분 미승인 자동 보류
...
// 승인 시스템 v2 tier 인가(OWNER 2026-07-08). normal(문구·코드) 승인자 풀 = author 아닌 풀 1인.
// core(위험·보안·삭제·배포)는 OWNER(lead) 탭만. OWNER 는 상위권한이라 normal 도 승인 가능(단 self-approve 금지).
// 에이전트 승인은 CLI/버스 경로(텔레그램 탭 아님). 소문자 id 비교.
// ★풀은 하드코딩 아니라 설정(setting)으로 언제든 변경(OWNER 2026-07-08, 데이터구조>분기 원칙).
...
/** 설정에서 normal tier 승인자 풀 조회(콤마/공백 구분, 소문자). 미설정=기본 4인. OWNER 가 대시보드/설정에서 변경. */
...
export function canApproveTier(opts: {
  ...
  isLead: boolean; // approver 가 OWNER(lead)로 이미 인증됐나 (core / 상위권한)
  ...
  // ① self-approve 금지 — 모든 tier. (lead 가 자기 요청을 승인하는 경우도 차단; 실무상 OWNER 는 요청자가 아님)
  if (approver && author && approver === author) {
    return { ok: false, reason: "self-approve 금지(author≠approver)" };
  }
  if (opts.isLead) return { ok: true }; // OWNER 는 normal·core 둘 다 승인 가능(상위권한)
  if (opts.tier === "core") return { ok: false, reason: "core tier 는 팀장(OWNER) 승인만" };
  // normal: 풀 자격(설정 주입)
...
/**
 * 승인 시스템 v2(OWNER 2026-07-08): maxAgeMinutes(기본 10분) 이상 pending 인 승인을 '보류(deferred)' 로
...
// PIN per-session — 검증 1회 후 짧은 TTL 토큰으로 재요구 없이(OWNER UX). in-memory(프로세스).
...
/**
 * 신뢰된 탭 승인(텔레그램 인라인 버튼). 인증 = 호출부에서 확인한 OWNER 텔레그램 id(=tapper).
 * PIN 없이 실행 — OWNER의 검증된 탭 자체가 인가(채널 텍스트보다 강함: callback_query 는 위조 불가). 2026-06-10 OWNER.
 */
