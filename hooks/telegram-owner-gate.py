...
배경:
  member(claude_channel, requireMention=false)은 그룹 전체 메시지를 자기 텔레그램 봇(plugin)으로
  직접 받는다(경로 A). 이 경로엔 team-collab 라우터의 owner-gate 가 없어서, member이 자기 것이
  아닌 메시지(@member 등)에도 "판단"으로 응답해버리는 게 반복됐다(2026-06-02 진단).
  이 hook 은 들어온 그룹 메시지를 라우터(/api/route)에 보내 owner 를 확인하고,
  member이 owner 가 아니면 그 prompt 를 block 해서 응답 자체를 막는다(판단 의존 제거).
...
v1 범위 (안전 우선):
  ...
  - 확신 케이스만 block: 라우터 reason=="explicit_mention" 이고 내가 targets 에 없을 때
    (= @다른사람만 호출한 메시지. 예: "@member …" → member 침묵). 오늘 T2 케이스.
  - 그 외(reply_author/sticky/default_intake/broadcast/ask_owner/라우터 에러·타임아웃)는
    전부 fail-open 통과. 팀 원칙 "false-drop(무응답) > over-summon" — 불확실하면 막지 않는다.

v2 (후속):
  - reply/sticky 게이팅: 경로 A엔 reply 원문/sticky 가 없으니, 캡처가 버스에 적재한
    reply_to_agent + activeAssigneeId 를 끌어와 /api/route 에 넣어 T4(member 답장→codex) 등도 차단.
...
설치 (터미널 = self-mod, 채널 OK로는 분류기가 막음):
  ...
  member poller 재시작 후 T1~T6 로 라이브 검증.
...
def main():
    ...
    # owner/suppress 룰은 전부 서버(teamRouter.shouldSuppress)에 있다 — 이 훅엔 자체 로직 없음(OWNER 2098).
    try:
