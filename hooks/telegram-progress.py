...
def _react_owner_skip(chat_id, text, msg_id=""):
    ...
    owner/suppress 룰은 서버 한 곳(teamRouter.shouldSuppress). DM·비그룹·에러는 False(=👀 유지, fail-open). [GD 2098]"""
...
def handle_pre(state_dir, data):
    ...
    # (gd 요청 순서: "작업 중" 삭제 → 답변. Stop 불발과 무관하게 신뢰성 확보.)
    ...
    # '작업한 방'에 분리해 띄움. 없으면(주입/태그없음) 기존 폴백 + 주입 skip. (2026-05-27 GD: 방별 분리)
