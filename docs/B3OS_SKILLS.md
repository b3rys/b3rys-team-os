...
| **b3os-project-mgmt** | **skills** | ★프로젝트 관리·보고★ — 팀장 요구가 여러 번 쌓이는 일에서 지시를 빠짐없이 목록(TODO.md 정본)으로 잡고 칸반과 맞추며, 채널별(텔레그램 아이콘 정리·슬랙·팀버스·저장소) 정해진 모양으로 보고. 시간 규칙(작업 상태로 진행 · 작업 단위 10분 + 병렬)과 검증 사다리(기준 먼저 → 헤드리스 → 리뷰 → 통합 → 릴리스 검사 → 전달). "목록으로 관리해" "정리해서 보내봐" "다 처리했어?". `references/channel-formats.md`. owner=steve |
