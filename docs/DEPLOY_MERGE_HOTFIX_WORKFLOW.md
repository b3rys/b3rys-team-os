...
배포는 화면이 뜨는 것으로 끝나지 않는다. 바꾼 것이 라이브에서 실제로 도는지 확인한다.
직접 못 건드리는 대상이면 가장 가까운 방법으로 확인하고, 무엇을 못 쟀는지 남긴다.

#### 규칙 정본 대조

배포 게이트(`--mode deploy`)가 규칙 정본 `rules/TEAM-OS.md` 와 공개 템플릿을 대조한다.

`rules/TEAM-OS.md` 는 추적 대상이 아니라 배포와 함께 오지 않는다. 템플릿만 온다.
그래서 규칙이 바뀌어도 받는 쪽은 모른다. 이 대조가 그것을 알린다.

다르다고 배포가 실패한 것은 아니다. 고치지도 않는다 — 팀 사정에 맞춘 차이일 수 있다.
공개본의 새 규칙이면 그 줄만 정본에 옮긴다.

```bash
scripts/rules-drift-check.sh   # 같으면 0, 다르면 1 과 diff
```
