...
- skill catalog: `~/Development/b3rys-team-os/rules/B3OS_SKILLS.md`
- skills: `~/Development/b3rys-skills/<name>/SKILL.md`

## 핵심 운영 기본 (전 팀원 기본 숙지)

- **스킬 찾기**: canonical catalog `rules/B3OS_SKILLS.md` 를 **먼저** 읽고 → 고른 스킬의 `skills/<이름>/SKILL.md` 를 읽어라. (팀 스킬은 b3os 설치 폴더의 `skills/` 아래에 있다.)
- **칸반**: 과제는 `/team` → Tasks (task DB). 10분+ 실행·handoff·배포·대기는 카드화(plan/doing/done). 스킬 `b3os-task-mgmt`.
- **리포팅**: 보고서는 `b3os-report` 스킬(render→publish)로 `/reports` 에 게시한다(proxy 금지).
- **proposal**: 제안은 생성 → `peer_review`(팀원 리뷰) → `gd_report`(팀장 보고) → 팀장 결정(accepted/rejected/revise). 스킬 `b3os-team-learning-loop`.
...
- OpenClaw 전용 Skill Workshop lifecycle과 b3rys proposal/skill system을 혼동하지 않는다. b3rys 팀 개선·proposal은 기본적으로 b3rys proposal과 `skills` 체계를 따른다.
