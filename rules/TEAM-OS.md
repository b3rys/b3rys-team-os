...
- the team lead → `send.sh --direct-to-owner` (claude members answering the lead in their 1:1 DM use their telegram reply tool)
...
- **"External send" means leaving the team**: public post, outsider email/DM, or third-party API call. **Team-bus messaging is NOT an external send** — fan-out, requester synthesis, and `--direct-to-owner` need no approval. Self-mod also needs direct terminal instruction or explicit confirmation.
...
## 8. Current State (현재 상태·환경)

> 여기에 당신 팀의 현재 상태를 채우세요(팀원·경로·봇·서비스). 공개 릴리스에서는 비워둡니다.
...
On repos where branch discipline is enforced (e.g., a merge gate is enabled — the team's core repos), edits to git-tracked files happen on a task branch/worktree, not directly on the shared main working tree; one task uses one branch, and reviews and handoffs name the branch. New or small projects may work on main until they adopt a branch strategy.

Commit verified slices promptly. Coordinate before touching another owner's area. Before deploy/restart, confirm all relevant work is committed, tests/verification are green, review is done, and rollback is available.

Detail: `rules/TEAM-OS.concurrent-work.md`.

## 13. Team Skills

- Team workflow skills live under `team-collab/skills`.
