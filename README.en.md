...
To have several teammates work together in **one group chat**, add a **System OP bot** (the team router — it reads **every** message in the group and routes it to the right teammate):

1. **Create the op bot** — make a **separate** bot with @BotFather (not one of your member bots) and get its token.
2. **Create the group and invite** — make a Telegram **group**, invite the op bot and each member bot.
3. **★Turn OFF the op bot's privacy mode — the most commonly missed step★** — @BotFather → `/setprivacy` → pick the op bot → **Disable**. *(Why: with privacy mode ON (BotFather's default) a bot only sees commands, its own @mentions, and replies to itself — it **never receives ordinary group messages**, so un-mentioned instructions and other bots' mentions don't reach the router. Alternative: promote the op bot to **group admin** — admins bypass privacy mode.)*
4. In the dashboard **Settings ▸ System OP**: paste the **capture bot token** (use **Check bot connection** to confirm the token), set the **group chat_id** (with the op bot in the group, post any message, then tell Claude Code *"what's our group chat_id?"* — it auto-detects and sets it; a single discovered group is set automatically), and toggle the **router ON**.

**Verify**: send `@teammate` or `/status` in the group — a reply means you're set. If auto-detect finds no group, re-check step 3 (privacy mode). With the router **OFF** (default), teammates don't respond in the group (decisions are shadow-logged only) — turn it **ON** for group collaboration. You can also set this up later by telling Claude Code *"how do I set up the b3os group room?"*.
