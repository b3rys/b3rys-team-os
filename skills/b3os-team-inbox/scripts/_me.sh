...
#
# 해석 순서:
#   1) OWNER_AGENT_ID 환경변수 — 명시적 override (레거시 이름 유지, OWNER 2026-07-10)
...
#    (낡은/유출된 OWNER_AGENT_ID 가 올바른 폴더 판별을 조용히 덮어쓰는 것을 막는다. 하네스 F3)
if [ -n "${OWNER_AGENT_ID:-}" ]; then
  if [ -z "$(q "SELECT 1 FROM agent WHERE id = '$(sq "$OWNER_AGENT_ID")' LIMIT 1;")" ]; then
    echo "✖ OWNER_AGENT_ID='$OWNER_AGENT_ID' 는 팀에 없는 id 다. 해석을 중단한다." >&2
    exit 1
  fi
  echo "$OWNER_AGENT_ID"
  exit 0
...
{
  echo "✖ agent id 해석 실패."
  echo "  시도: OWNER_AGENT_ID → 현재 폴더($(pwd -P)) ↔ agent.workspace_path → tmux 세션(${SESSION:-없음})"
