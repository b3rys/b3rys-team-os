#!/usr/bin/env bash
# bus-recall.sh — 1:1 세션에서 '팀버스 맥락'을 쉽게 조회 (OWNER 2026-07-09).
#
# 문제: openclaw/hermes 는 1:1 방과 팀버스가 다른 세션이라, 1:1 에서 "팀버스 그거 어떻게 됐어?"·
#   "OWNER가 member한테 뭐 시켰어?" 같은 질문을 받으면 버스 맥락을 몰라 다 뒤진다(토큰 낭비).
# 해결: 이 스크립트로 team.db(팀버스=message + OWNER 1:1=dm_message)를 한 번에 SQL 조회해 요점만 본다.
...
#   성공처럼 돌려주면 호출자가 "별 거 없네" 하고 넘어간다. 틀린 답 대신 멈춘다. (OWNER 2026-07-10)
# --with 는 '나 ↔ 상대' 쌍 조회라 ★신원이 필수★ 다. 신원 없이 돌리면 매칭이 0건이 되어
#   "둘이 대화한 적 없음" 처럼 보이는 조용한 false-negative 가 된다(Devon 리뷰, 2026-07-10).
#   반면 --about(주제) · --from-owner <member>(대상 지정) 는 신원 없이도 의미가 성립한다.
...
if [ -z "$ME" ] && [ -z "$FROM_GD" ] && [ -z "$ABOUT" ]; then
  {
    ...
    echo "  이 상태로 진행하면 '내 OWNER 1:1' 이 빠진 결과가 나와서 맥락을 잃는다."
    echo "  해결: 자기 워크스페이스 폴더에서 실행하거나 --me <agent id> 를 붙여라."
    echo "  (범위를 직접 지정하는 --about / --from-owner <member> 는 신원 없이도 동작한다."
...
# 2) OWNER 1:1 (dm_message) — 나 또는 지정 멤버
...
if target:
    ...
    print(f"\n=== OWNER 1:1 ({target}, dm_message) — {len(drows)}건 ===")
    for r in reversed(drows):
        who = "OWNER→" if r['direction']=='in' else f"{target}→OWNER"
        print(f"  [{r['created_at'][11:16]}] {who}: {(r['body'] or '')[:70]}")
    if not drows: print("  (없음 — 해당 멤버 OWNER 1:1 캡처 없음)")
con.close()
print("\n(read-only 조회 — 이걸로 '뒤지기' 대신 요점 확인. 팀버스=message, OWNER 1:1=dm_message)")
PY
