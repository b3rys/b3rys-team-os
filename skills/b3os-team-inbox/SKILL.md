...
**절차**
1. 각 팀원에게 fan-out 위임을 보낸다. **`--thread` 는 요청이 도착한 그 thread 를 그대로 쓴다.**
   (같은 thread 를 써야 답들이 그 요청에 묶인다)
...
3. 종합을 요청자에게 보낸다:
   `send.sh --to <요청자> --thread <같은 thread>` (팀장께 직접이면 `--direct-to-owner`)

**주의: 턴 본문은 아무 데도 안 간다.** 보낸 것만 도달한다 — `send.sh` 를 부르지 않으면 종합은 존재하지 않는다.

```bash
# ★수집 요청이 도착한 thread 를 그대로 쓴다★ (새로 만들지 않는다)
TH="<수집을 지시받은 그 메시지의 thread_id>"

skills/b3os-team-inbox/scripts/send.sh --to steve --thread "$TH" \
