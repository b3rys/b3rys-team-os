...
### ★띄웠으면 반드시 회수한다★ (실측 2026-08-12)

**서브를 띄우는 것과 결과를 받는 것은 다른 호출이다.** 띄우기만 하고 턴을 끝내면 **결과가 없다.**

| 런타임 | 띄우기 | ★회수★ |
|---|---|---|
| codex | `spawn_agent` (비동기 — id 만 즉시 온다) | ★`wait_agent`★ (안 부르면 결과 없음) · 목록 `list_agents` |
| claude | Workflow/Agent | 그 호출이 결과까지 준다 |

**실제로 이걸로 통째로 날렸다** — dex 가 `spawn_agent` 3개를 띄우고 *"서브 결과를 회수합니다"* 라고
말한 뒤 **`wait_agent` 를 부르지 않고 턴을 끝냈다.** 2분 작업이 산출물 0으로 끝났고,
그 뒤 프로세스가 닫히며 서브 3/4 가 완료 전에 죽었다.

**규약**
- 띄운 수와 **회수한 수를 세서 맞춘다.** 안 맞으면 그 사실을 보고에 적는다(조용한 누락 금지)
- 회수에 **시간 상한**을 준다(`timeout_ms`). 무한정 기다리면 턴이 안 끝난다
- 상한에 걸린 조각은 **"미회수" 로 보고**한다 — 없는 것처럼 넘기지 않는다
- **말로 "회수하겠습니다" 는 회수가 아니다.** 도구를 실제로 불러야 한다
