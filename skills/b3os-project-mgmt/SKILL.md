...
- 모든 일은 ★10분 단위★ 로 쪼개고, 순서가 필요 없는 것은 ★같은 10분에 동시에★ 한다. 사람이 모자라면 보조 에이전트를 붙인다. 부품은 파일로 나눠 서로 부딪히지 않게 설계한다.
- 시간 예측은 단계를 늘어놓고 더하지 않는다. 진짜 의존과 순서만 세고, 예측에는 "(10분 단위 작업 병렬 산출)" 이라고 적는다.
- 예외는 따로 적는다: 순서가 꼭 필요한 것(한 번에 하나만 쓸 수 있는 자원 — 예: 앱 창을 쓰는 검사), 외부 요소(공증·다운로드·외부 서비스 대기 등).
- 위임한 순간 다음 일을 시작한다 — 보고를 기다리며 비우지 않는다.
