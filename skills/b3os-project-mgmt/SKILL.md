...
프로젝트를 열 때 에이전트 팀을 설계하고(만드는 역할 N · 판정하는 역할 · 통합하는 역할 + 공통부 주인 1명), 진행 단계마다 관리한다(각도가 늘면 역할 추가, 한 역할이 두 일을 하면 분리, 병목이면 주인 이동, 단계 전환이면 재설계). 역할 정의는 프로젝트 저장소 `.claude/agents/` 에 파일로. 도메인 예시: 프로그램 개발 · 영상 · 콘텐츠 · 금융/제조 분석. 짜는 법의 정본은 `b3os-harness-playbook`.
