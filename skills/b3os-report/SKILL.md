...
> ★화면을 옮기는 코드를 두지 마라.★ 2026-09-07 하루에 이 자리에서 결함이 넷 났다 — 패널 첫머리 목표가 sticky 경계와 여유 0(1px 만 어긋나면 탭 줄이 내려앉음) · 닫힌 패널의 좌표가 0 이라 목표가 음수가 되어 문서 맨 위로 튐 · 복원에 바닥을 대서 읽던 자리가 아래로 밀림 · 저장값이 0 이라 복원이 맨 위로 감. 넷 다 "브라우저가 옮긴 화면을 스크립트가 따라다니며 고친다" 는 구조에서 나왔다. 옮기지 않으면 그 구조가 없어진다.
...
> ★`requestAnimationFrame` 을 쓰지 마라.★ 배경 탭(`document.hidden === true`)에서는 **콜백이 아예 실행되지 않는다**(실측 2026-09-02). 그 탭에서는 코드가 한 줄도 실행되지 않는데 화면이 비슷해 보여 **동작하는 것처럼 읽힌다.**
