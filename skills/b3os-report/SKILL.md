...
세 스킬이 보고서 작업에 붙는다. **셋 다 이 저장소에 없는 외부 스킬**이고 `install.sh` 가 설치하지도 않는다 — 없으면 그 단계를 사람이 직접 하면 된다. 설치돼 있으면 아래 자리에서 쓴다.

| 스킬 | 언제 | 무엇을 한다 | 무엇을 안 한다 |
|---|---|---|---|
| `gd-el10` | **쓰기 전** — 어떻게 설명할지 정할 때 | 되물음이 날 만한 설명(새 이름·원인·동작)을 한 번에 전달되게 하는 규칙 다섯 | 문장을 다듬지 않는다. 사실 확인도 안 한다 |
| `gd-explain-like-5` | **쓰는 중** — 문단 하나가 어려울 때 | 그 문단 아래 5줄 이내 풀이 박스: 원래 표현 → 쉬운 해석 → 왜 중요한지 | 보고서 전체 구조에는 안 쓴다 |
| `humanize-korean` | **렌더 직전** — 마지막 패스 | 번역투·기계적 병렬·AI 티 나는 리듬만 다듬는다 | **내용을 바꾸지 않는다.** 숫자·용어·인용은 그대로 |

순서가 겹치지 않는다 — `gd-el10` 은 **무엇을 어떤 순서로 말할지**, `gd-explain-like-5` 는 **막히는 한 문단**, `humanize-korean` 은 **다 쓴 뒤 문체**다. 셋을 같은 자리에서 돌리면 서로 결과를 덮는다.
