...
- **/reports 게시는 컨펌 없이 진행한다**. `/reports` 는 팀 내부 포털이라 approval gate 대상이 아니다. **팀 밖으로 나가는 것(공개 포스팅·외부 이메일/DM·서드파티 API)은 그대로 승인 대상이다.**

## 문체 원칙 — humanize-korean 최종 패스
...
3. **팀원 리뷰 — 내용과 표현 둘 다** (★건너뛰지 않는다★)
   ...
   **자동 검사는 이 다섯 갈래를 못 잡는다.** `humanize-korean` 의 71개 패턴에 대응하는 것은 `E-4 단문 일변도` 하나뿐이고,
   지시관형사 축·지어낸 표현 축은 목록에 없다. 정량 점수에 위 예문들을 넣으면 **위험도 낮음(2점)** 이 나온다(실측).
...
5. **포털 게시** — ★원본 md/html 은 `reports/` 밖(작업 폴더·임시 폴더)에 두고 publish.sh 가 복사하게 한다.★
   이미 `reports/<id>/` 안에 써 두면 자기 자신을 복사하다 `SameFileError` 로 죽는다(실측).
...
**★탭을 눌러도 화면을 옮기지 않는다★** — 링크는 남기되 스크립트가 있으면 클릭을 가로챈다(`e.preventDefault()`). 패널은 클래스로 바꿔 끼우고 주소는 `history.replaceState(null,'','#tab=<이름>')` 로 조용히 바꾼다. 브라우저의 앵커 이동이 안 일어나므로 **읽던 자리가 그대로 남는다.**

스크립트가 켜지면 `.wrap` 에 `js-tabs` 를 붙여 `:target` 규칙을 끈다. 두 규칙이 같이 살아 있으면 패널이 둘 보인다.

> ★화면을 옮기는 코드를 두지 마라.★ 이 자리에서 나는 결함은 넷이다 — 패널 첫머리 목표가 sticky 경계와 여유 0(1px 만 어긋나면 탭 줄이 내려앉음) · 닫힌 패널의 좌표가 0 이라 목표가 음수가 되어 문서 맨 위로 튐 · 복원에 바닥을 대서 읽던 자리가 아래로 밀림 · 저장값이 0 이라 복원이 맨 위로 감. 넷 다 "브라우저가 옮긴 화면을 스크립트가 따라다니며 고친다" 는 구조에서 나온다. 옮기지 않으면 그 구조가 없어진다.

**깊은 앵커만 한 번 옮긴다** — `#어느-장` 으로 들어오면 그 장이 든 패널을 켠 뒤 그 자리로 한 번 옮긴다. 패널을 켠 뒤에야 좌표가 정해지므로 순서를 지킨다. `scrollTo` 호출은 **이 한 곳뿐**이고 `behavior:'instant'` 다.

> ★`window.scrollTo(x, y)` 2인자 형태를 쓰지 마라.★ `theme.css` 가 `scroll-behavior:smooth` 를 켜 놓아서 이 형태는 **화면을 못 움직인다**(실측: 목표 5,270 지정, 결과 제자리). `window.scrollTo({top, behavior:'instant'})` 를 쓴다. 에러가 안 나므로 코드를 넣은 것이 고쳤다는 증거로 잘못 읽히기 쉽다.

> ★`requestAnimationFrame` 을 쓰지 마라.★ 배경 탭(`document.hidden === true`)에서는 **콜백이 아예 실행되지 않는다**(실측). 그 탭에서는 코드가 한 줄도 실행되지 않는데 화면이 비슷해 보여 **동작하는 것처럼 읽힌다.**
