...
**스크립트가 하나 붙는데, 스크롤만 거든다.** 목적지가 **닫혀 있던 탭 안**이면 브라우저는 첫 스크롤 시점에 그 요소의 위치를 모른다 — 화면에 없어서다. CSS 가 탭을 편 뒤에도 브라우저는 다시 시도하지 않는다. 실측 — 스크립트를 뺀 판으로 `#p3-5` 를 열면 목표에서 **6,180px** 떨어진 곳에 선다. 스크립트가 있으면 0.
