...
★탭은 이동이지 효과가 아니다★ — 렌더러가 탭 보고서에 `html{scroll-behavior:auto}` 를 넣어 `theme.css` 의 부드러운 스크롤을 끈다. 켜 두면 탭을 눌렀을 때 브라우저 애니메이션과 보정 이동이 다투어 **화면이 앞뒤로 흔들린다**(실측 2026-09-02). 보정도 탭 클릭에서는 **한 번만** 움직인다. 깊은 앵커일 때만 400ms 뒤에 어긋난 폭이 4px 를 넘는지 확인해 그때만 다시 옮긴다. ★여러 번 옮기면 그 자체가 떨림이다.★
