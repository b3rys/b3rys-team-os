...
| 중요 설계·빌드 결정 | **multi-ai review** | `owner-multi-ai-review` |
...
- **openclaw·hermes 런타임 팀원**: SKILL.md 자동로드가 없을 수 있다 → BWF 과제면 이 스킬을 **명시 로드**한다(경로 = `<repo>/skills/b3os-bwf/SKILL.md`). 카탈로그는 `rules/B3OS_SKILLS.md`에서 발견. 핵심 단계는 위 6줄 표를 ready-run으로 바로 따라 실행.
