...
**적용 범위(scope guard — 먼저 읽기)**: BWF는 **팀장 컨펌을 받은 팀 실행 과제·위임 과제**에 적용한다. 단순 답변·가벼운 조회·의견·즉시 끝나는 국소 작업에는 풀 절차를 강제하지 않는다 — TEAM-OS §4/§10 "작은 일" 기준대로 thread에 owner·근거만 남기면 충분하다. (이걸 안 박으면 openclaw 런타임에서 작은 질문까지 PM계획→카드로 끌려간다.)
