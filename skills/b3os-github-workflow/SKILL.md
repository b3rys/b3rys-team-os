...
PR 작성자가 승인자와 **달라야** 리뷰 요건이 성립한다. 그래서 PR은 팀원 작업용 계정으로 만든다.

```bash
GH_TOKEN="$(gh auth token --user "<팀 계정>")" gh pr create --base main --fill
```

**확인 — 작성자가 팀 계정으로 찍혔는지:**

```bash
gh pr view <번호> --json author --jq .author.login
```

> ⚠️ 토큰을 화면에 출력하거나 명령 인자로 남기지 않는다. 위처럼 **명령 치환으로 바로 주입**한다. `gh auth token` 결과를 파일이나 로그에 남기지 않는다.
>
> ⚠️ `GH_TOKEN` 은 `gh` 명령에만 적용된다. **일반 `git push` 는 이 변수를 쓰지 않는다** — push 계정을 바꿀 목적으로 이 변수를 쓰면 조용히 다른 계정으로 나간다.
