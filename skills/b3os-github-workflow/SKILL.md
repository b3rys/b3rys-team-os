...
**실측(2026-07-29, 최근 PR 45건·승인 35건): 이름 있음 17 / 이름 없음 18 = 51%.**
본문은 다들 충실했다 — 리뷰를 안 한 게 아니라 **이름만 안 적었다.**

### 머지 전에 '누가 봤는지'를 눈으로 본다

```bash
bash scripts/pr-who-approved.sh <PR번호>          # 보여주기만
bash scripts/pr-who-approved.sh <PR번호> --gate   # 이름 없는 승인이 있으면 exit 1
```

**왜 승인 쪽이 아니라 머지 쪽인가** — 2026-07-29 에 #117 을 머지하면서
"저 도장은 Demis 것" 이라고 알고 있었는데 **내 것이었다.** 그런데 그 승인 본문 첫 줄은
**`Bill 승인합니다` 였다 — 이름은 있었다.** 즉 그날의 실패는 *이름을 안 쓴 것*이 아니라
**머지 전에 승인을 읽지 않은 것**이다. 규율을 더 요구해도 안 고쳐진다(이미 지켜져 있었으므로).
그래서 확인 지점을 **읽는 쪽에** 둔다.

'누가 봤는지'를 잘못 알고 머지를 판단하면 **다음엔 아무도 안 본 것을 머지하게 된다.**
