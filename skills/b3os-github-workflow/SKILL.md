...
trigger: branch·PR·merge · writing anything that lands in the repo
---
...
Submitted-by: <팀원 ID>
```
(★리뷰 본문 마지막 줄★ 에) `Approved-by: <승인자 ID>`

발견자·검증자 크레딧(`발견 = <팀원 ID>(교차검증)`)은 **이것과 별개**다. 그건 사람이 읽는 정보라 본문 안에 자유롭게 쓴다.

### 리뷰 승인은 ★리뷰 본문 마지막 줄★ 에 이름을 쓴다

우리 승인은 계정이 공유라 **기록만으로는 누가 했는지 구분되지 않는다.** 그래서 승인에
**기계가 읽는 한 줄**을 남긴다 — **마지막 줄**에 정확히 이 모양으로:

```
Approved-by: <승인자 ID>
```

**★여기서 말하는 `Approved-by:` 는 PR 리뷰 도장이다.★** 권한 승인(permission request) 쪽에도
같은 문자열이 있다(`src/server/lib/approvals.ts` — 승인 결과에 실제 승인자를 남기는 트레일러).
★이름만 같고 다른 제도다.★ `grep Approved-by` 하면 둘이 한 화면에 섞여 나오니 어느 쪽인지 보고 쓴다.

> #### ⚠️ ★어디에 쓰느냐가 절반이다 — PR 설명글이 아니라 리뷰다★
>
> 게이트는 PR 본문(description)을 **안 본다.** `release-preflight.sh` 가 읽는 것은
> `repos/<slug>/pulls/<번호>/reviews` 다 — ★리뷰 API★ 다.
...
> ```
>
> `approver-check.py` 를 따로 부르지 않는다. 그건 ★그 안의 판정부★ 이고
> (시험을 네트워크 없이 돌리려고 떼어둔 것), ★진입점은 preflight 하나★ 다.
>
> **★제목·본문·예시가 같은 자리를 가리켜야 한다.★** 제목이 본문과 다른 줄을 말하면 그 자체가 결함이다 —
> 읽는 사람은 제목만 보고 따른다. 그리고 "승인 본문" 이라고만 쓰면 ★설명글로 읽힌다.★
> ★어느 본문인지 매번 적는다.★
