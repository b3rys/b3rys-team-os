...
> 예: *"만들 팀원의 id를 정해주세요 — 영문 소문자 slug (예: `alex`, `sam`). 표시이름·역할도요. `@`로 부를 **멘션 별칭**이 더 있으면 알려주세요(id·표시이름은 자동 등록). 그리고 **성격·말투·전문성 같은 persona**를 한두 줄로 정해줄래요? (선택 — 지금 비워도 되고, 나중에 대시보드 Settings에서 넣을 수 있어요.)"*
