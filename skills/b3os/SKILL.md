...
**셋업 (사람: 봇 1개 + 그룹 chat_id + privacy mode OFF). 각 단계에 "왜 / 흔한 실패 / 확인법"을 붙였다:**
...
2. **그룹 생성 + 봇 초대** — 팀원들을 넣을 **텔레그램 그룹**을 만들고 **op 봇 + 각 팀원 봇**을 초대한다. *(확인: 그룹 멤버 목록에 op 봇과 팀원 봇이 보이면 됨.)*

3. **★op 봇 privacy mode 끄기 — 가장 자주 빠뜨리는 단계, 안 하면 라우팅이 통째로 안 된다★** — @BotFather → `/setprivacy` → op 봇 선택 → **Disable**.
   - **왜**: privacy mode 가 켜져 있으면(BotFather 기본값 = **Enabled**) 봇은 그룹에서 **자기에게 온 명령(`/cmd`)·자기 멘션·자기 답장만** 받고 **일반 그룹 메시지는 못 받는다**. 그러면 capture 봇이 그룹 대화를 못 읽어 라우팅(멘션 없는 지시·다른 팀원 멘션)이 전혀 동작하지 않는다.
   - **대안**: op 봇을 그룹 **관리자(admin)로 승격**해도 된다 — 텔레그램 admin 은 privacy mode 를 우회해 전체 메시지를 받는다. (privacy Disable 이 더 간단하고 확실하다.)
...
4. **대시보드 Settings ▸ 시스템 OP (System OP)** 에서 —
   ...
   - **(4-b) 그룹 chat_id 설정 (자동감지 권장)** — op 봇이 그룹에 든 상태에서 그룹에 **아무 메시지나 하나** 올린 뒤, **Claude Code에게 "우리 그룹 chat_id 알려줘"** 라고 하면 자동으로 찾아 설정한다(내부: `POST /system-op/detect-group` — capture 워커가 이미 본 그룹을 조회하고, **그룹이 하나면 자동 지정**·둘 이상이면 목록을 주고 고르게 한다). `no_group_seen` 이 뜨면 → op 봇이 그룹에 있는지 + **3번 privacy mode 를 껐는지** + 그룹에 메시지를 하나 보냈는지 확인 후 재시도. 수동으로 넣어도 된다 — 그룹에 잠깐 **@userinfobot**(또는 @RawDataBot)을 초대하면 chat_id(대개 `-100…`)를 알려준다.
   - **(4-c) Router ON** (시스템 OP 토글 또는 `.env` `ROUTER_ENABLED=true`). ★라우터 토글은 즉시 반영★(재시작 불필요).

5. **동작 확인** — 그룹에 `@팀원` 멘션 또는 `/status` → 응답이 오면 팀방 협업 준비 완료. 응답이 없으면: ①라우터가 ON 인지 ②op 봇 privacy mode 를 껐는지(3번) ③그룹 chat_id 가 맞게 잡혔는지 순으로 본다.
