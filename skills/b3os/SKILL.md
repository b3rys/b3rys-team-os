...
git clone https://github.com/b3rys/b3rys-team-os.git "$B3OS"
cd "$B3OS"
bash install.sh
```
...
5. (선택) **팀장 텔레그램 chat_id**(`owner_chat_id`, 숫자) — 외부/BYO 런타임의 발신자 게이트용. 설정 안 하면 그 봇이 낯선 사람 DM에도 응답할 수 있다.
...
> - **외부/BYO 런타임(발신자 게이트 필요)**: [4]의 `owner_chat_id`(팀장 chat_id)를 먼저 넣는다 — 없으면 봇이 낯선 사람 DM에도 응답한다(fail-open). 이건 외부/BYO 런타임 발신자 게이트용이며 claude 페어링과는 별개다.
...
> 참조하면 되고(온디맨드), b3os-team-os 저장소 코드는 건드리지 않는다. 영입 시
